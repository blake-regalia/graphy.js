...
function Parser(z_input, h_config) {
	...
	Object.assign(this, {
		...
		// term constructors
		blank_node: (s_label) => new BlankNode(s_label),
		anonymous_blank_node: (s_label) => new AnonymousBlankNode(s_label),
		named_node: (s_iri) => new NamedNode(s_iri),
	...
	if(null === z_input) {
		...
		// graceful error
		this.grace = (e_parse) => {
			d_transform.emit('error', e_parse);
		};
