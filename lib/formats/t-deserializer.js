...
function Parser(z_input, h_config) {
	...
	// split tracking
	let k_split_tracker = this.split_tracker = null;
	if('split_tracking' in h_config) {
		...
		let {
			targets: a_targets,
			reached: fk_reached,
		} = h_config.split_tracking;
		k_split_tracker = this.split_tracker = new split_tracker(a_targets, fk_reached);
	}
	...
	// stream
		...
		z_input.on('data', (s_chunk, n_bytes) => {
			...
			// split tracking; thanks to special readable we know how many bytes are in this chunk
			if(k_split_tracker) {
				let nl_chunk = s_chunk.length;
				k_split_tracker.upcoming(n_bytes, nl_chunk, n - nl_chunk);
			}
...
Object.assign(Parser.prototype, {
	...
	@{method('post_object')}
		...
		@{else_if_char('.')}
			...
			@if TTL
				// split tracking; mark character position
				if(this.split_tracking) this.split_tracking.terminal(i);
				@{goto(STARTING_STATE)}
			@else
