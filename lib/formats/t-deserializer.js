...
const R_IRIREF = /<([^>]*)>\s*/y;
const R_PREFIXED_NAME_ESCAPELESS = 		/([^\s#:<\[("'_][^\s#:<\[("']*)?:((?:[^\s#<\[("'.;,)\]\\](?:[^\s#<\[("';,)\]\\]*[^\s#<\[("'.;,)\]\\])?)?)(?:\s+|(?=\.[<\[("';,)\]#\s]|[<\[("';,)\]#]))/y;
//										/([^\s#:<\[("'_][^\s#:<\[("']*)?:([^\s#<\[("'.;,)\]\\]*)(?:\s+|(?=[<\[("';,)\]#]))/y;
// const R_PREFIXED_NAME_ESCAPELESS_WITH_STOPS = /([^#:<\[("'_][^\s#:<\[("']*)?:((?:[^\s#<\[("'.;,)\]\\]*[^\s#<\[("';,)\]\\])?)(?:\s+|(?=[<\[("'.;,)\]#]))/y;

const R_PREFIXED_NAME = 				/([^\s#:<\[("'_][^\s#:<\[("']*)?:((?:(?:[^\s#<\[("'.;,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"]))(?:(?:[^\s#<\[("';,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"]))*(?:[^\s#<\[("'.;,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"])))?)?)(?:\s+|(?=\.[<\[("';,)\]#\s]|[<\[("';,)\]#]))/y;
@ // ([^\s#:<\[("'_][^\s#:<\[("']*)?: 			# 1: prefix id
@ // (											# 2: prefix iri
@ // 	(?: 									
@ // 		(?: 								# [first name char]
@ // 			[^\s#<\[("'.;,)\]\\] 			# [not invalid name char]
...
@ // 	)?
@ // )
@ // (?:\s+|(?=\.[<\[("';,)\]#\s]|[<\[("';,)\]#])) 	# [consume trailing whitespace]

const R_PREFIXED_NAME_EOF = 			/([^\s#:<\[("'_][^\s#:<\[("']*)?:((?:(?:[^\s#<\[("'.;,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"]))(?:(?:[^\s#<\[("';,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"]))*(?:[^\s#<\[("'.;,)\]\\]|\\(?:%[0-9A-Fa-f][0-9A-Fa-f]|[^\s"])))?)?)(?:\s*\.\s*$)/y;
...
function Parser(z_input, h_config) {
	let h_state = h_config.state || {};

	// track index for anonymous blank node labels
	let i_anon = h_state.blank_node_index || 0;
		...
		if(this.@{STARTING_STATE} !== this.state) {
			...
			if(this.datatype === this.state || this.object_list === this.state) {
				...
				@{if_match('R_PREFIXED_NAME_EOF', 'm_prefixed_name_eof')}
					@{valid_prefix('m_prefixed_name_eof')}

					// commit iri from resolve prefixed name
					let y_node = new NamedNode(this.prefixes[s_prefix_id] + m_prefixed_name_eof[2]);

					// object
					if(this.object_list === this.state) {
						this.object = y_node;
					}
					// datatype
					else {
						this.object.datatype = y_node;
					}

					// emittance and cleanup
					@{end_of_statement()}
				@{end_else()}
			}
...
Object.assign(Parser.prototype, {
	...
	@{method('datatype_or_langtag')}
		...
		@{if_char('^', '@')}
			...
			@{else_if_match('R_LANGTAG', 'm_langtag')}
				...
				this.object@{literal_datatype()} = HP_NN_RDFS_LANG_STRING;
	...
	@{method('datatype')}
		...
		@{if_match('R_IRIREF_ESCAPELESS', 'm_iriref_e_datatype')}
			...
			this.object@{literal_datatype()} = new NamedNode(p_datatype);
		...
		@{else_if_match('R_PREFIXED_NAME_ESCAPELESS', 'm_prefixed_named_e_datatype')}
			...
			// set literal datatype
			this.object@{literal_datatype()} = new NamedNode(this.prefixes[s_prefix_id] + m_prefixed_named_e_datatype[2]);
		...
		@{else_if_match('R_PREFIXED_NAME', 'm_prefixed_named_datatype')}
			...
			// set literal datatype
			this.object@{literal_datatype()} = new NamedNode(this.prefixes[s_prefix_id] + s_suffix);
