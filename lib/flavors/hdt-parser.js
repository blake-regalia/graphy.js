...
Object.assign(Parser.prototype, {
	...
	@{method('crc8', 1)}
		if(this.crc !== a[i++]) {
			this.parse_error('CRC8 validation check failed after '+this.nested.pop()+'; expected '+'0x'+a[i-1].toString(16)+', instead got 0x'+this.crc.toString(16));
		}
		else {
			this.crc = 0;
			@{pop_state()}	
	...
	@{method('crc16', 2)}
		...
		if(this.crc !== x_crc_check) {
			this.parse_error('CRC8 validation check failed after '+this.nested.pop());
		}
		else {
			this.crc = 0;
			@{pop_state()}	
	...
	dict_sect_front_coding_string_prep(a, n, i) {
		this.crc = -1;
		@{goto('dict_sect_front_coding_string')}
	},
	...
	@{method('triples_bitmap_bits')}
		let n_bytes_needed = Math.ceil(this.bit_length / 8);
		if(@{at_least('n_bytes_needed')}) {
			let a_bytes = a.slice(i, i + n_bytes_needed);

			// compute crc
			let x_crc = this.crc;
			@{crc_range(32, 'i + n_bytes_needed')}
			this.crc = x_crc;
	...
	@{crc_state('triples_bitmap_array_preamble', 8, 'triples_bitmap_array_entries')}
