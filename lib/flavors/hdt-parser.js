...
const D_UTF_8_TEXT_ENCODER = new TextEncoder('utf-8');
const D_UTF_16LE_TEXT_ENCODER = new TextEncoder('utf-16le');
...
const encode_utf_16 = B_ENV_NODE
	? (s_chunk) => {
...
const D_UTF_8_TEXT_DECODER = new TextDecoder('utf-8');
const D_UTF_16LE_TEXT_DECODER = new TextDecoder('utf-16le');

const decode_utf_8 = B_ENV_NODE
	? (ab_chunk) => {
		return ab_chunk.toString('utf-8');
	}
	: (ab_chunk) => {
		return D_UTF_8_TEXT_DECODER.decode(ab_chunk);
	};

const decode_utf_16le = B_ENV_NODE
	? (ab_chunk) => {
		return ab_chunk.toString('utf-16le');
	}
	: (ab_chunk) => {
...
@macro string_to_buffer(str, nullify_var)
	/[^\u0000-\u00ff]/.test(@{str})? encode_utf_16(@{str}): encode_utf_8(@{str})
	@if nullify_var
		;
		// free string to GC
		@{str} = null;
...
function Parser(z_input, h_config) {
	...
	if(z_input.setEncoding) {
		...
		z_input.on('data', (a_chunk) => {
			...
			// begin
			let a_next_state = this.state(a, n, 0);
			while(a_next_state) {
				a_next_state = a_next_state[0].apply(this, a_next_state.slice(1));
			}
	...
	@{method('dict_sect_front_coding_string')}
		...
		while(i < n) {
			...
			if(s_term[0] === '"') {
				...
				let ab_content = @{string_to_buffer('s_content')};
			...
			// non-literal
				...
				ab_word = @{string_to_buffer('s_word')};
			...
			if(b_fragmented) {
				...
				if(v8 && this.approaching_heap_limit(this.bytes_consumed)) {
					...
					if(this.graph.idx_sp) {
						...
						let ab_idx_merge = @{mk_uint_array('a_idxs.length + ab_idx_sp.length', 'i_soal')};
