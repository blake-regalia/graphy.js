...
const D_UTF_16LE_TEXT_ENCODER = B_ENV_NODE? null: new TextEncoder('utf-16le');
...
const D_UTF_16LE_TEXT_DECODER = B_ENV_NODE? null: new TextDecoder('utf-16le');
...
function Parser(z_input, h_config) {
	...
	if(z_input.setEncoding) {
		...
		z_input.on('data', (a_chunk) => {
			...
			if(h_config.progress) h_config.progress(this.offset, this.state.name);
		});
