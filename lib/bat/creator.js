...
class term_loader {
	...
	load(s_mime, dfb_input, fc_terms, fk_terms) {
		...
		k_worker.run({
			...
			events: {
				dump: (a_triples) => {
					...
					this.save(a_triples);
					t_previous_dump = performance.now() - t_start;
				},
			},
		}, () => {
			this.process();
...
class creator {
	...
	constructor(h_config) {
		...
		let k_term_loader = new term_loader({
			workers: worker.group('./worker.js'),
		});
