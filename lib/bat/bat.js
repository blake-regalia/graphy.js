...
class buffer_writer {
	...
	append(at_item) {
		...
		let nl_grow = nl_contents;
		while(nl_grow < i_end) {
			nl_grow = nl_grow + N_DEFAULT_ALLOCATION_SIZE;
		}

		// alloc new buffer and copy contents
		if(nl_grow !== nl_contents) {
			let at_alloc = new Uint8Array(nl_grow);
			at_alloc.set(at_contents);
			at_contents = at_alloc;
		}
...
class word_reader {
	constructor(ab_contents, at_indices) {
		Object.assign(this, {
	...
	*words() {
		...
		for(let i_word=0; i_word<n_words; i_word++) {
			...
			yield ab_contents.slice(i_read, i_end);
			i_read = i_end;
		}
	}
	...
	*triples() {
		...
		for(let i_word=0; i_word<n_words; i_word++) {
			let i_end = at_indices[i_word++];
			let ab_s = ab_contents.slice(i_read, i_end);
			i_read = i_end;

			i_end = at_indices[i_word++];
			let ab_p = ab_contents.slice(i_read, i_end);

			i_end = at_indices[i_word++];
			let ab_o = ab_contents.slice(i_read, i_end);
			i_read = i_end;
...
class front_coder {
	...
	export() {
		...
		let n_contents_word = n_words - (h_defer? h_defer.indices.length: 0);
	...
	add(ab_fragment, a_indices) {
		...
		if(!k_contents.write) {
			...
			if(0 !== i_offset % n_block_size) {
				...
				this.defer = {
					contents: ab_fragment.slice(0, i_range_end),
		...
		if(0 !== n_words % n_block_size) {
			// decode final word in current block
			let i_read_block = a_block_indices[a_block_indices.length-1];
			let at_contents = k_contents.contents;

			// start with head word
			let i_end = at_contents.indexOf(0, i_read_block);
			let at_word = at_contents.slice(i_read_block, i_end);

			// decode remainder of block
			// let c_words_block = 1;
			// let n_words_block = n_words % n_block_size;
			// for(; c_words_block<n_words_block; c_words_block++) {
			let k_bus_decoder = new bus.decoder(at_contents);
			k_bus_decoder.read = i_read_block;
			for(; k_bus_decoder.read<k_contents.write; ) {
				let n_shared = k_bus_decoder.pint();
				let nl_code = k_bus_decoder.pint();
				let at_code = k_bus_decoder.sub(nl_code);
		...
		while(i_read < nl_fragment) {
			...
			// write byte length of word
			k_contents.append(bus.encode_pint(nl_word));

			// plainly encode head word in its entirety
			k_contents.append(ab_word);
			...
			while(i_read < nl_fragment && (c_words_block++ < n_block_size)) {
				...
				// write rest of word
				k_contents.append(ab_test.slice(c_shared, c_shared+nl_code));
