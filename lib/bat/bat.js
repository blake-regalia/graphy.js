...
class word_reader {
	constructor(ab_content, at_indices) {
		Object.assign(this, {
			content: ab_content,
			indices: at_indices,
		});
	...
	*words() {
		...
		for(let i_word=0; i_word<n_words; i_word++) {
			...
			yield ab_content.slice(i_read, i_end);
			i_read = i_end;
		}
	}
	...
	*triples() {
		...
		for(let i_word=0; i_word<n_words; i_word++) {
			let i_end = at_indices[i_word++];
			let ab_s = ab_content.slice(i_read, i_end);
			i_read = i_end;

			i_end = at_indices[i_word++];
			let ab_p = ab_content.slice(i_read, i_end);

			i_end = at_indices[i_word++];
			let ab_o = ab_content.slice(i_read, i_end);
			i_read = i_end;
