...
class front_coder {
	...
	close(x_code_chapter) {
		...
		let k_output = new buffer_writer({size:n_byte_estimate});

		// chapter code
		k_output.append(bus.encode_pint(x_code_chapter));

		// word count
		k_output.append(bus.encode_pint(n_words));

		// block size k
		k_output.append(bus.encode_pint(Math.log2(n_block_size, 2)));

		// block indices length
		k_output.append(bus.encode_pint(nl_block_indices));

		// block indices
		k_output.append(at_block_indices);

		// contents length
		k_output.append(bus.encode_pint(nl_contents));

		// contents
		k_output.append(k_contents.close());

		return k_output.close();
	}
