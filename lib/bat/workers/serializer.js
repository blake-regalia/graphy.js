...
class loader {
	...
	list_prefixed_items(k_creator) {
		...
		let k_front_coder = this.front_coder();
		k_front_coder.add_nt_word_list(at_contents, n_prefixes);

		// commit chapter
		this.push_chapter(k_front_coder.close(P_ENCODING_CHAPTER+S_PREFIXES));
	...
	load_sync(pm_format, k_stream, k_notify, fc_terms, fk_terms) {
		...
		graphy.deserializer(pm_format, k_stream, {
			...
			// eof
				...
				let k_tasks = this.process_terms(k_creator);
	...
	commit_dictionary() {
		...
		// dictionary code
		kbe.ntu8_string(bat.P_ENCODING_DICTIONARY);
	...
	process_terms(k_creator) {
		...
		this.uti_map = bus.new_uint_array(i_uti, i_uti);
		...
		let k_tasks = k_workers
			...
			.reduce({
				...
				if(!h_word_buffer_nodes_absolute) {
					...
					k_workers.wait('uti_'+S_TERM_HP, () => {
						h_utis.subjects_absolute = h_utis.hops_prefixed;
						k_workers.unlock(['uti_'+S_TERM_SA, S_TERM_SA]);
					});

					k_workers.wait('uti_'+S_TERM_SP, () => {
						h_utis.objects_absolute = h_utis.subjects_prefixed;
						k_workers.unlock(['uti_'+S_TERM_OA, S_TERM_OA]);
					});
				}
				// yes absolute nodes
				else {
					...
					for(let s_key in h_word_buffer_nodes_absolute) {
						...
						k_tasks = k_tasks
							...
							.sequence((h_fragment_nodes_absolute) => {
								k_front_coder.import(h_fragment_nodes_absolute);
							}, () => {
								let s_chapter = H_CHAPTERS_ABSOLUTE[s_key];

								// push this chapter to dictionary
								this.push_chapter(k_front_coder.close(P_ENCODING_CHAPTER+s_chapter));

								// absolute nodes section done
								k_workers.unlock(s_chapter);
							});
					}
			...
			.reduce({
				...
				if(!h_word_buffer_nodes_prefixed) {
					...
					k_workers.wait('uti_'+S_TERM_HA, 'uti_'+S_TERM_HP);
					k_workers.wait('uti_'+S_TERM_SA, 'uti_'+S_TERM_SP);
					k_workers.wait('uti_'+S_TERM_PA, 'uti_'+S_TERM_PP);
					k_workers.wait('uti_'+S_TERM_OA, 'uti_'+S_TERM_OP);

					k_workers.wait(S_TERM_HA, S_TERM_HP);
					k_workers.wait(S_TERM_SA, S_TERM_SP);
					k_workers.wait(S_TERM_PA, S_TERM_PP);
					k_workers.wait(S_TERM_OA, S_TERM_OP);
				}
				// yes prefixed nodes
				else {
					// save & unlock uti maps
					k_workers.wait('uti_'+S_TERM_HA, () => {
						h_utis.hops_prefixed = this.add_uti_map(h_word_buffer_nodes_prefixed.h.ids, h_utis.hops_absolute);
						k_workers.unlock('uti_'+S_TERM_HP);
					});

					k_workers.wait('uti_'+S_TERM_SA, () => {
						h_utis.subjects_prefixed = this.add_uti_map(h_word_buffer_nodes_prefixed.s.ids, h_utis.subjects_absolute);
						k_workers.unlock('uti_'+S_TERM_SP);
					});

					k_workers.wait('uti_'+S_TERM_PA, () => {
						h_utis.predicates_prefixed = this.add_uti_map(h_word_buffer_nodes_prefixed.p.ids, h_utis.predicates_absolute);
						k_workers.unlock('uti_'+S_TERM_PP);
					});

					k_workers.wait('uti_'+S_TERM_OA, () => {
						h_utis.objects_prefixed = this.add_uti_map(h_word_buffer_nodes_prefixed.o.ids, h_utis.objects_absolute);
						k_workers.unlock('uti_'+S_TERM_OP);
					});
					...
					for(let s_key in h_word_buffer_nodes_prefixed) {
						...
						k_tasks = k_tasks
							...
							.sequence((h_fragment_nodes_prefixed) => {
								...
								k_workers.wait(H_CHAPTERS_ABSOLUTE[s_key], () => {
		...
		// build task group for literals
		({tasks:k_tasks} = this.literals_task_groups(k_tasks, {
			creator: k_creator,
			seed: S_TERM_OP,
			utis: h_utis,
		}, [
			...
			{
				chapter: S_TERM_LDA,
				data: this.balance_literals_datatyped_absolute(k_creator),
			},
			{
				chapter: S_TERM_LDP,
				data: k_workers.balance(this.literals_datatyped_prefixed),
			},
		]));

		// once the last chapter is ready
		k_workers.wait(['uti_'+S_TERM_LDP, S_TERM_LDP], () => {
			// commit dictionary
			this.commit_dictionary();

			// process triples
			this.process_triples(k_creator);
	...
	literals_task_groups(k_tasks, h_groups, a_groups) {
		let {
			creator: k_creator,
			seed: s_dependency,
			utis: h_utis,
		...
		a_groups.forEach((h_group) => {
			let s_chapter = h_group.chapter;

			k_tasks = this.add_task_group(k_tasks, {
				creator: k_creator,
				data: h_group.data,
				dependency: s_dependency,
				chapter: s_chapter,
				front_code_args: [n_block_size_k_literals],
				utis: h_utis,
			});

			s_dependency = s_chapter;
		});

		return {
			tasks: k_tasks,
			chapter: s_dependency,
		};
	}

	add_task_group(k_tasks, h_sequence) {
		let {
			creator: k_creator,
			data: a_data,
			dependency: s_dependency,
			chapter: s_chapter,
			front_code_args: a_args_front_code=[],
			utis: h_utis,
		} = h_sequence;

		let k_workers = this.workers;
		let i_uti = k_creator.uti;
	...
	process_triples(k_creator) {
		...
		let at_adj_sp_o = bus.new_uint_array(h_utis.literals_datatyped_prefixed, n_triples);
		let kbs_sp_o = new bus.bitsequence_writer(n_triples);
		...
		let at_adj_s_p = bus.new_uint_array(h_utis.predicates_prefixed, n_pairs);
		let kbs_s_p = new bus.bitsequence_writer(n_pairs);
...
// debugger;
		...
		let kbe_header = new bus.buffer_encoder({size:512});

		// encoding scheme
		kbe_header.ntu8_string(bat.P_ENCODING_TRIPLES_BITMAP+'spo');
...
debugger;
	...
	close_output() {
		...
		// encode header
		let kbe_header = new bus.buffer_encoder({size:512});
		kbe_header.ntu8_string(bat.P_ENCODING_DATASET);
		...
		// write output
		let dfb_output = new Blob(this.output, {type:'application/octet-binary'});
		let p_output = URL.createObjectURL(dfb_output);

		// resolve worker promise
		this.resolve([p_output]);
	}
}
