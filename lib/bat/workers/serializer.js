...
class loader {
	...
	list_prefixed_items(k_creator) {
		...
		// commit chapter
		this.push_chapter(k_front_coder.close(PE_CHAPTER+S_PREFIXES));
	...
	process_terms(k_creator) {
					...
					for(let s_key in h_word_buffer_nodes_absolute) {
						...
						k_group
							...
							.sequence((h_fragment_nodes_absolute) => {
								...
								this.push_chapter(k_front_coder.close(PE_CHAPTER+s_chapter));
		...
		k_group
			...
			.data(k_group.balance(this.nodes_prefixed))
...
worker.dedicated({
	...
	load_stream(pm_format, ds_file) {
		return new Promise((f_resolve) => {
			...
			k_creator.load_sync(pm_format, ds_file, this);
		});
	},
