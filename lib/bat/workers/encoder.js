...
// debugger;
		...
		let at_ids = bus.new_uint_array(i_uli_max, nl_words);
	...
	merge_word_buffers(h_a, h_b, i_uli_max, b_pause) {
		// if(b_pause) debugger;
		...
		let at_ids_c = bus.new_uint_array(i_uli_max, at_ids_a.length+at_ids_b.length);
	...
	merge_word_lists(h_a, h_b) {
		...
		let at_indices_c = bus.new_uint_array(nl_content_c, n_words_c);
	...
	front_code_prefixed_nodes(h_classes) {
		...
		return worker.result(h_front_coders);
	},
