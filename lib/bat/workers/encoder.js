...
module.exports = function(self) {
	worker.dedicated({
		...
		front_code_word_buffer(h_buffer) {
			// start front coder off at the right offset
			let k_front_coder = new bat.front_coder({
			...
			// front code fragment
			k_front_coder.add_nt_word_list(h_buffer.contents, h_buffer.word_count);
		...
		classify_sort_concat_encode(a_words, i_uli_max) {
			// let t_sort_last = performance.now();
			let h_types = bat.classify_nodes_list(a_words);
			// t_sort_last = Math.round(performance.now() - t_sort_last);

			// let t_sort_first = performance.now();
			// a_words.sort(F_SORT_VALUE);

			// let h_types_a = bat.classify_nodes_list_no_sort(a_words);
			// t_sort_first = Math.round(performance.now() - t_sort_first);
		...
		concat_encode(a_words, i_uli_max) {
			...
			// 	k_buffer_writer.append(D_TEXT_ENCODER.encode(a_literals[i_literal]));
			// }
...
// debugger;
			...
			let at_ids = bat.new_uint_array(i_uli_max, nl_words);
		...
		sort_concat_encode(a_words, i_uli_max) {
			// console.time('sort strings');
			// a_words.sort();
			// console.timeEnd('sort strings');

			console.time('sort objects');
			a_words.sort(F_SORT_VALUE);
			console.timeEnd('sort objects');

			return this.tasks.concat_encode(a_words, i_uli_max);
		},

		merge_word_buffers(h_a, h_b, i_uli_max) {
			...
			if(!i_uli_max) debugger;

			let nl_a = at_a.length;
			let nl_b = at_b.length;
			let nl_c = nl_a + nl_b;
			let at_c = new Uint8Array(nl_c);
			let at_ids_c = bat.new_uint_array(i_uli_max, at_ids_a.length+at_ids_b.length);
			...
			// sanity check
			if(i_write_c !== nl_c) {
				throw new Error('bug alert: an assertion failed; sorted null-terminated word buffers were not thoroughly merged');
			}
		...
		merge_terms_triples(h_creator_a, h_creator_b) {
			return new bat_creator(h_creator_a)
				.import(h_creator_b)
				.export();
		},
	});
