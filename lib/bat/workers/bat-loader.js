...
class loader {
	...
	import_front_coder_fragment_nodes(h_front_coders_import) {
		debugger;
	}
			...
			.reduce({
				...
				// yes absolute nodes
					...
					return k_workers
						...
						.dispatch({
							task: 'front_code_word_buffer',
							data: this.balance_word_buffer(h_word_buffer_nodes_absolute),
						})
						// combine front-coded fragments
						.sequence((h_fragment_nodes_absolute) => {
							this.import_front_coder_fragment_nodes(h_fragment_nodes_absolute);
						}, () => {
							// absolute nodes section done
							k_workers.unlock('nodes_absolute');

							console.info('absolute nodes');
						});
				}
			})
			...
			.reduce({
				...
				// yes prefixed nodes
					...
					k_workers.wait('uti_nodes_absolute', () => {
						...
						i_s_p = this.add_uti_map(h_word_buffer_nodes_prefixed.s.ids, i_s_a);
						i_p_p = this.add_uti_map(h_word_buffer_nodes_prefixed.s.ids, i_p_a);
						i_o_p = this.add_uti_map(h_word_buffer_nodes_prefixed.s.ids, i_o_a);
					...
					// new task stream
					return k_workers
						// front code prefixed buffer nodes
						.dispatch({
							task: 'front_code_word_buffer',
							data: this.balance_word_buffer(h_word_buffer_nodes_prefixed),
						})
						// combine front-coded fragments
						.sequence((h_fragment_nodes_prefixed) => {
							this.import_front_coder_fragment_nodes(h_fragment_nodes_prefixed);
						}, () => {
							// prefixed nodes section done
							k_workers.unlock('nodes_prefixed');

							console.info('prefixed nodes');
						});
				}
			})
