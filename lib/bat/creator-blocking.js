...
class term_loader {
	...
	load_sync(s_mime, dfb_input, fc_terms, fk_terms) {
		let t_start = performance.now();

		let k_blob = new readable_blob(dfb_input, {
...
class creator {
	...
	constructor(h_config) {
		...
		let k_term_loader = new term_loader({
			workers: worker.group('./workers/encoder.js'),
		});

		k_term_loader.load_async(...f_mk_terms(), (h_term_pass) => {
