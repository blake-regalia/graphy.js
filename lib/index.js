...
const __construct = function(h_jsonld) {
	...
	**/
		...
		let h_properties = {
			...
			$: {
				value: Object.defineProperties(function(s_namespace) {
					...
					short: {
						get: () => {
							return '"'+h_literal.value.replace(/"/g, '\\"')+'"^^'+shorten(h_literal.datatype);
						},
					},

					// auto-prefix datatype
					datatype: {
						get: () => {
							return shorten(h_literal.datatype);
						},
					},
				}),
			},
	...
	const cover_collection = (h_collection, p_namespace) => {
		...
		// map_array_interface(k_collection, a_list, f_collection_item);
