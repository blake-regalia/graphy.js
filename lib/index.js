	...
	**/
	...
	const cover_collection = (h_collection, p_namespace) => {
		...
		let k_collection = rmprop(function(f_each) {
			...
			// this is an implicit map call
				...
				return a_list_covered.map((k_item) => {
					return f_each(k_item);
				});
			}
			// other
			else {
				local.fail('calling a collection is an implicit `map` call on the underlying list; method requires a callback function, instead got: '+arginfo(f_each));
			}
		});
		...
		if(P_RDF_NAMESPACE === p_namespace) {
			...
			k_collection.rest = a_rest.length? cover_collection({type: 'collection', list: a_rest}, p_namespace): cover_iri(H_IRI_RDF_NIL, P_RDF_NAMESPACE);
		}
