...
//
const map_array_interface = (a_input, f_map) => {
	...
	A_ITERATION_METHODS.forEach((s_method) => {
		...
		a_input[s_method] = function() {

			// first, map each value to network node
			let a_mapped = d_proto.map.call(this, f_map);
...
const fetch_object = (z_value) => {
	...
	if(Array.isArray(z_value)) {
		...
		local.warn(`more than one triple share the same predicate "${shorten(p_predicate)}" in object "${s_id}". By using '.${s_access_name}', you are accessing any one of these triples arbitrarily`);
	...
	// object is literal
		...
		if(h_key_context['@type']) {
			...
			// datatype suggests that primitve conversion is possible
			if(H_PRIMITIVES[s_datatype]) {

				// set unboxing primitve datatype accessor
				h_object_access['$value'] = {
					value: H_PRIMITIVES[s_datatype](z_value),
				};
			}
		}
...
const __construct = function(h_jsonld) {
			...
			for(let s_key in h_node) {
				...
				// value type is object
					...
					// node is something else
						...
						local.fail('unrecognized object struct for json-ld node value: '+arginfo(z_value));
					}
				}
				// other
				else {
					local.fail('unrecognized datatype for json-ld graph node value: '+arginfo(z_value));
				}
			}
		});

		//
		local.good('top-level nodes:\n'+arginfo(a_top_level_nodes));
	...
	**/
	...
	const network_node = (h_node, p_namespace) => {
		...
		local.warn('creating network node on: '+arginfo(h_node));
					...
					h_properties[s_access_name] = {
						...
						get: () => {
							...
							if(a_nodes.length > 1) {
								...
								// issue warning about using first triple
								local.warn(`more than one triple share the same predicate "${shorten(p_predicate)}" in object "${h_node.id}". By using '.${s_access_name}', you are accessing any one of these triples arbitrarily`);
							}
		...
		const k_node = rmprop(function(s_suffix) {
			...
			return map_array_interface(a_nodes, (h_item) => {
				local.good('covering networked node: '+arginfo(h_item));
	...
	const cover = (p_node) => {
		...
		// set a method to allow user to create network node here
		f_query.network = (s_namespace, f_okay) => {

			// create network node from this json-ld node
			let k_node = network_node(h_node, expand(s_namespace))

			// for consistency, let user set callback
			if('function' === typeof f_okay) f_okay(k_node);

			// but also just return the node normally
			return k_node;
		};
	...
	**/
	...
	// set the default public operator to a list of top-level covered nodes
	const operator = top_nodes(a_graph);
	...
	**/
	...
	// extend the array with methods that give control over graph
	{
		...
		operator['network'] = function(s_namespace, f_each) {
			...
			// each top-level node in graph
			operator.forEach((h_node) => {

				// create a network node from this json-ld node
				let k_node = network_node(h_node, p_namespace);
