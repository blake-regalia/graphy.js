...
const R_NUMERIC_LITERAL = /([+\-]?(?:[0-9]+([.]?[0-9]*)|([.][0-9]+))([eE][+\-]?[0-9]+)?)\s*/y;
...
// special thanks to Ruben Verborgh for these two regexes
const R_IRI_ABSOLUTE = /^[A-Za-z][A-Za-z0-9.\-+]*:/;
const R_DOT_SEGMENTS = /(?:^|\/)\.\.?(?:$|[\/#?])/;
...
const F_DOT_SEGMENTS = (s_iri) => {
	if(!R_DOT_SEGMENTS.test(s_iri)) return s_iri;

	// optimized for schemed URLs (i.e., does not support scheme-less URLs)
	...
	for(let i=0; i<a_segments.length; i++) {
		...
		if('..' === s_segment) {
			if(a_output.length > i_path) a_output.pop();
		}
		else if('.' !== s_segment) {
			let x = s_segment[0];
			if('#' === x || '?' === x) {
				do {
					a_output.push(a_segments[i++]);
				} while(i < a_segments.length)
				break;
			}
			// empty segment means two consecutive slashes, at end of scheme segment
			else if(!s_segment && i === i_scheme) {
				i_path = i + 2;
			}
			// colon could indicate start of scheme
			else if(!i_scheme && ':' === s_segment[s_segment.length-1]) {
				i_scheme = i + 1;
			}
		...
		// relative iri
			...
			switch(s_iri[0]) {
				...
				case '?':
					...
					break;
				case '/':
					this.@{term} = @{object?'new Iri(':''}('/' === s_iri[1]? this.base_scheme: this.base_root) + F_DOT_SEGMENTS(s_iri)@{object?')':''};
					break;
				default:
					this.@{term} = @{object?'new Iri(':''}F_DOT_SEGMENTS(this.base_path + s_iri)@{object?')':''};
			}
			// // make @{term}
			// this.@{term} = @{object?'new Iri(':''}this.base + s_iri@{object?')':''};
		}
...
@macro numeric_literal()
	...
	if(m_numeric_literal[4]) {
		this.datatype = P_IRI_XSD_DOUBLE;
	}
	// contains decimal point, xsd:decimal
	else if(m_numeric_literal[2] || m_numeric_literal[3]) {
		this.datatype = P_IRI_XSD_DECIMAL;
	}
	// otherwise, it is an integer
	else {
		this.datatype = P_IRI_XSD_INTEGER;
	}
@end
...
@macro boolean_literal()
	...
	// xsd:boolean
	this.datatype = P_IRI_XSD_BOOLEAN;
@end
...
@ //		// prefix id
@ //		@{else_if_match('R_PREFIX', 'm_prefix', true)}
...
@ //
@ //		// base
@ //		@{else_if_match('R_BASE', 'm_base', true)} 
@ //			// set base iri
@ //			this.base = m_base[1];
@ //
@ //			// callback
...
@ //			continue;
		...
		@{else_if_match('R_BASE_KEYWORD', 'm_base_keyword')}
			...
			@else
				console.warn('[graphy] WARNING: The current TTL document uses relative IRIs. For better performance, call graphy parser with option `relative_iris` set to true')

				// load relative iri version
				Object.setPrototypeOf(this, ParserRelativeIris.prototype);
			@end
...
@ // now, create subclass that accounts for relative iris
@set RELATIVE_IRIS true

class ParserRelativeIris {
