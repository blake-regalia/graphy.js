/*eslint no-console:0*/
const fs = require('fs');

let p_file = '/Users/blake/Downloads/geo_coordinates_mappingbased_en.ttl';
// p_file = '/Users/blake/dev/volt-debug/data/source/dbpedia-places-california.ttl';
// p_file = '/Users/blake/dev/volt-debug/data/source/dbpedia-places-oregon.ttl';
p_file = '/Users/blake/Downloads/persondata_en.nt';
// p_file = '/Users/blake/Downloads/instance-types_en.nq';

let s_flavor = process.argv[2];

console.time('g');
let parser = require(`../${s_flavor}/parser`);

let c_triples = 0;
let c_literals = 0;
let c_iris = 0;
let c_literals_length = 0;



parser(fs.createReadStream(p_file), {
	// quad(h_quad) {
	// 	c_triples += 1;
	// 	if(h_quad.object.isLiteral) {
	// 		c_literals += 1;
	// 		c_literals_length += h_quad.object.value.length;
	// 	}
	// 	else if(h_quad.object.isNamedNode) {
	// 		c_iris += 1;
	// 	}
	// },
	triple(h_triple) {
		c_triples += 1;
		if(h_triple.object.isLiteral) {
			c_literals += 1;
			c_literals_length += h_triple.object.value.length;
		}
		else if(h_triple.object.isNamedNode) {
			c_iris += 1;
		}
	},
	end() {
		console.timeEnd('g');
		console.log(`${c_triples} triples parsed`);
		console.log(`${c_iris} named nodes`);
		console.log(`${c_literals} literals`);
		console.log(`... totaling ${c_literals_length} characters in total`);
		console.log(`${process.memoryUsage().rss / 1024 / 1024}MiB`);
	},
});





// const stream = require('stream');
// class Reader extends stream.Writable {
// 	constructor(...a_args) {
// 		super({

// 		});
// 	}
// 	_write(s_chunk, s_encoding, f_done) {
// 		console.log(s_encoding+': '+s_chunk);
// 		f_done();
// 	}
// }

// function streams(s_ttl) {
// 	let d_stream = new stream.Readable();
// 	let a_ttl = s_ttl.split('');
// 	d_stream._read = function() {
// 		this.push(a_ttl.shift() || null);
// 	};
// 	return d_stream;
// }


// streams('<alpha> <bravo> <charlie> .')
// 	.pipe(parser({
// 		triple(h_triple, a_buffer) {
// 			console.log('+1');
// 			c_triples += 1;
// 			if(h_triple.object.isLiteral) {
// 				c_literals += 1;
// 				c_literals_length += h_triple.object.value.length;
// 			}
// 			else if(h_triple.object.isNamedNode) {
// 				c_iris += 1;
// 			}
// 			a_buffer.push('<'+h_triple.subject+'> => '+h_triple.object);
// 		},
// 		end() {
// 			console.timeEnd('g');
// 			console.log(`${c_triples} triples parsed`);
// 			console.log(`${c_iris} iris`);
// 			console.log(`${c_literals} literals`);
// 			console.log(`... totaling ${c_literals_length} characters in total`);
// 			console.log(`${process.memoryUsage().rss / 1024 / 1024}MiB`);
// 		},
// 	}))
// 	.pipe(new Reader())
// 	.on('end', () => {
// 		console.log('end');
// 	});

