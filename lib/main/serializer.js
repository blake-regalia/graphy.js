...
class HumanReadableSerializer extends stream.Readable {
	...
	n3_node_terse(s_n3) {
		...
		// next blank node
		if(!s_n3.length) return '_:b'+(this.blank_node_index++);

		// full iri path
		if('<' === s_n3[0]) {
			// assume valid URI
			if('>' !== s_n3[s_n3.length-1]) {
				return s_n3.slice(1, -1);
			}
			// do escaping
			else {
				return s_n3.slice(1).replace(R_NAMED_NODE_CHARS_INVALID, s => '\\U'+(s.codePointAt(0)).toString(16).padStart(8, '0'));
			}
		}
