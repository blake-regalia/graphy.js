...
class HumanReadableSerializer extends stream.Readable {
	...
	n3_node_terse(s_n3) {
		...
		// find delimiter
		let i_colon = s_n3.indexOf(':');
		if(-1 === i_colon) throw 'subject must be n3 string';
	...
	n3_term(s_n3) {
		...
		if(-1 !== i_quote) {
			...
			// not plain
				...
				throw 'invliad n3 string/literal';
			}
...
class QuadsWriter {
	...
	add(h_quads) {
		...
		for(let s_graph in h_quads) {
			...
			// create graph node
			let k_graph = this.graph(s_graph);

			// value is function
			if('function' === typeof z_triples) {
				// call with graph
				z_triples(k_graph);
			}
			// value is hash
			else {
				k_graph.add(z_triples, 0);
			}
		}
	}
}
...
class TriplesWriter extends Writer {
	...
	add(h_triples, x_mask=4) {
		...
		for(let s_subject in h_triples) {
			...
			// create pairs writer
			let k_subject = this.subject(s_subject);

			// value is function
			if('function' === typeof z_pairs) {
				// call with subject
				z_pairs(k_subject);
			}
			// value is hash
			else {
				k_subject.add(z_pairs, x_mask);
			}
...
class PairsWriter extends Writer {
	...
	add(h_pairs, x_mask) {
		...
		for(let s_predicate in h_pairs) {
			...
			// create objects writer
			let k_predicate = this.predicate(s_predicate);

			// value is function
			k_predicate.add(w_objects, x_mask);
...
class ObjectsWriter extends Writer {
	...
	serialize(z_objects, x_mask, n_nest_level=1) {
		...
		// coercable instance
		else if(z_objects.constructor in hm_coercions) {
