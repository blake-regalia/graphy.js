...
class PathLeg {
	...
	id_of_v(s_n3) {
		...
		@{dict_find('o')}
			console.log(`${s_n3} => ${k_graph.count_d + k_graph.count_s + c_item_o + 1}`);
		@{dict_else('o')}
...
class Bag extends PathLeg {
	...
	match(s_n3) {
		...
		if('"' === s_n3[0]) {
			...
			// find every vertex that has instance of literal's value
			let a_vertexes = [];
			let i_literal_base = k_graph.count_d + k_graph.count_o + 1;
			@{dict_find('l')}
				console.log('found instance @'+c_item_l);
				a_vertexes.push(i_literal_base + c_item_l);
			@{dict_else('l')}

			//
			return this._match_any(a_vertexes);
		}
		// node
		else {
			@{compress_n3_node()}
...
@ //			@{dict_else('s')}
		...
		// 
		throw `could not match "${s_n3}" because no such element exists [${a_word}]`;
	}
