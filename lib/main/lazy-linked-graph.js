...
@macro each_edge()
	// search sp's adjacency list for predicate
	let c_offset_data_p = -1;
	let i_data_p_start = k_graph.idx_p[i_source - 1];
...
@macro each_object()
	...
	while(true) {
		// pull up object's id
		let i_test_o = k_graph.data_o[i_data_o++];
	...
	cross(z_edge) {
		...
		if('string' === typeof z_edge) {
			...
			@{dict_find('p')}
				...
				//
				return new Edge(k_graph, a_pattern);
			@{dict_else('p')}
	...
	probe(z_probes, b_optimize_probe_first) {
		...
		// probe is hash
			...
			for(let s_probe_edge in z_probes) {
				...
				h_probes[i_p] = a_pattern_frag;

				// fire probe callback
				f_probe(new Edge(k_graph, a_pattern_frag));
			}
		}
...
@macro handle_probes()
	...
	@{find_probe_edge()}
		...
		let a_pattern_frag = h_probes[i_test_p];
...
debugger;
		...
		// recurse
		this._build(a_pattern_frag, a_collect);
...
class Selection {
	...
	rows() {
		...
		//
		let a_rows = [];
		return this._build(a_pattern, a_rows);
	}

	_build(a_pattern, a_rows) {
		let k_graph = this.graph;
...
@ // 
	...
	style() {
		...
		for(let i_source of f_sources()) {
			for(let [i_edge, c_offset_data_p] in f_edges(i_source)) {
				if(f_p_test(i_edge)) {
					for(let i_sink in f_sinks(i_source, c_offset_data_p)) {
						if(f_o_test(i_sink)) {
							f_o_act(i_sink);
						}
					}
