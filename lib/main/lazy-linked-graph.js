	...
	for(let i_test_@{c}=1; i_test_@{c}<=n_@{c}; i_test_@{c}++) {
		...
		while(true) {
			...
			while(true) {
				...
				// // don't fuck up the set's original insert order
				// if(!as_group_@{a}_@{b}@{c}_n.has(i_test_@{c})) {
...
debugger;
					// probe all of c
					this.probe(k_context, a_rows, h_sim_c);
				}
				...
				// more pattern to match
					...
					// proceed on c
					this.proceed(k_context.copy(), a_rows, h_row_c, h_sim_c);
				}
			}
		}
	}
...
class Selection {
	...
	rows() {
		//
		let a_rows = [];
		this.consume(this.context, a_rows);
		return a_rows;
	}
	...
	consume(k_context, a_rows) {
		...
		// no probes
			...
			this.proceed(k_context, a_rows, {}, h_head);
		}
	}
