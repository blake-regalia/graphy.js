...
const AB_UTF_16_TOKEN = Buffer.from([0]);
...
const decode_pint = bus.decode_pint;
...
const search_fc_block = (h_section, ab_word, i_offset, i_block) => {
	let {
		dict: ab_dict,
		ref: at_ref,
		k: n_block_k,
	...
	// set dict position
	let i_dict = at_ref[i_block << n_block_k];

	// jump to end of head word
	i_dict = ab_dict.indexOf(0, i_dict);
...
debugger;
	...
	for(let i_block_idx=1; i_block_idx<n_block_size; i_block_idx++) {
		...
		if(i_dict >= 0x80) [n_share, i_dict] = bus.decode_pint(ab_dict, i_dict);
		else i_dict++;
		i_dict += n_share;

		// fetch word's length; skip over word
		let nl_code = ab_dict[i_dict];
		if(i_dict >= 0x80) [nl_code, i_dict] = bus.decode_pint(ab_dict, i_dict);
		else i_dict++;
		...
		if(nl_test === nl_word) {
			...
			let i_char = nl_word - 1;
			do {
				if(ab_test[i_dict+i_char] !== a_word[i_char]) continue jump_search;
			} while(--i_char >= 0);

			// found the match
			return i_offset + i_block_idx + 1;
		}
	}
...
const search_fc_dict = (h_section, ab_word, i_offset) => {
	let {
		dict: ab_dict,
		ref: at_ref,
		k: n_block_k,
	...
	let i_lo = 0;
	let i_hi = a_ref[a_ref - 1] - 1;
	binary_search:
	for(;;) {
		// successfully isolated block
		if(i_lo === i_hi) {
			// search block for word
			return search_fc_block(h_section, ab_word, i_offset, i_lo);
		}
		// failed to find word
		else if(i_lo > i_hi) {
			return 0;
		...
		// search for block
			...
			let i_test = a_ref[i_mid];
			let x_test = a_dict[i_test];
			...
			// hit first character
				...
				while(i_char < n_word) {
					let x_test_n = a_dict[i_test+i_char];
					let x_word_n = a_word[i_char++];
				...
				else if(i_char < n_test) {
					i_hi = i_mid - 1;
					continue binary_search;
				}
				// found exact word!
				else {
					return i_offset + (i_mid << n_block_k) + 1;
				}
			}
		}
	}
...
const produce_fc_word = (k_section, i_term, i_offset=1) => {
	let {
		dict: ab_dict,
		ref: at_ref,
		k: n_block_k,
...
class LinkedGraph {
	...
	word_to_node(ab_word) {
		...
		// named node w/ prefixed name
			...
			// find prefix token
			let i_prefix_token = ab_word.indexOf(0x01);
	...
	clean_prefixes() {
		...
		for(let s_prefix in k_graph.prefixes) {
			...
			{
				let a_dict = k_graph.dict_s;
				let a_ref = k_graph.ref_s;
				scanning:
				for(let i_entry=0; i_entry<k_graph.count_s; i_entry++) {
					let c_chars = 0;
					for(let i_pos=a_ref[i_entry]; i_pos<a_ref[i_entry+1] && c_chars<n_prefix; i_pos++) {
						if(a_dict[i_pos] !== a_prefix[c_chars++]) continue scanning;
					}
			...
			{
				let a_dict = k_graph.dict_p;
				let a_ref = k_graph.ref_p;
				scanning:
				for(let i_entry=0; i_entry<k_graph.count_p; i_entry++) {
					let c_chars = 0;
					for(let i_pos=a_ref[i_entry]; i_pos<a_ref[i_entry+1] && c_chars<n_prefix; i_pos++) {
						if(a_dict[i_pos] !== a_prefix[c_chars++]) continue scanning;
					}
			...
			// {
			// 	a_prefix = encode_utf_8('^'+s_prefix+'\u0001');
	...
	s(i_subject) {
		...
		if(i_subject < this.range_d) {
			let ab_word = produce_fc_word(this.section_d, i_subject);
			return this.word_to_node(ab_word);
		}
		// subject
		else if(i_subject < this.range_s) {
			let ab_word = produce_fc_word(this.section_s, i_subject, this.range_d);
			return this.word_to_node(ab_word);
		}
		//
		else {
	...
	p(i_predicate) {
		let ab_word = produce_fc_word(this.section_p, i_predicate);
	...
	o(i_object) {
		...
		if(i_object < this.range_d) {
			let ab_word = produce_fc_word(this.section_d, i_object);
			return this.word_to_node(ab_word);
		}
		// object
		else if(i_object < this.range_o) {	
			let ab_word = produce_fc_word(this.section_o, i_object, this.range_d);
			return this.word_to_node(ab_word);
		}
		// literal
		else {
			if(i_object === 228) debugger;
			let ab_word = produce_fc_word(this.section_l, i_object, this.range_o);
			...
			if(94 === x_primer) {
				k_literal.datatype = decode_utf_8(ab_word.slice(1, i_content));
			}
	...
	find_s(s_n3) {
		...
		return search_fc_dict(this.section_d, ab_word)
			|| search_fc_dict(this.section_s, ab_word, this.range_d);
	}
	...
	find_p(s_n3) {
		...
		return search_fc_dict(this.section_p, ab_word);
	}
	...
	find_o(s_n3) {
		...
		if(s_n3.indexOf('"') === -1) {
			...
			return search_fc_dict(this.section_d, ab_word)
				|| search_fc_dict(this.section_o, ab_word, this.range_d);
		}
		else {
			// encode content
			...
			return search_fc_dict(this.section_l, ab_word, this.range_o);
		}
...
class EmptyPath extends PathLeg {
	...
	source(s_n3) {
		...
		// searchs duals dict
		let i_item_d = search_fc_dict(k_graph.section_d, ab_word);
		if(i_item_d) {
			k_context.append_id(i_item_d, HP_SUBJECT);
			return new Sources(k_graph, k_context);
		}

		// search subjects dict
		let i_item_s = search_fc_dict(k_graph.section_s, ab_word, k_graph.range_d);
		if(i_item_s) {
			k_context.append_id(i_item_s, HP_SUBJECT);
			return new Sources(k_graph, k_context);
		}
	...
	sink(s_n3) {
		...
		let i_item_d = search_fc_dict(k_graph.section_d, ab_word);
		if(i_item_d) {
			k_context.append_id(i_item_d, HP_OBJECT);
			return new Bag(k_graph, k_context);
		}

		// search objects dict
		let i_item_s = search_fc_dict(k_graph.section_o, ab_word, k_graph.range_d);
		if(i_item_s) {
			k_context.append_id(i_item_s, HP_OBJECT);
			return new Bag(k_graph, k_context);
		}
	}
	...
	literal(s_content, z_datatype_or_lang) {
		...
		// join parts into word
		let ab_word = join_buffers(encode_utf_8(s_open), ab_content);

		// searchs literals dict
		let c_item_l = search_fc_dict(k_graph.section_l, ab_word, k_graph.range_o);
		
		// found item
		if(c_item_l) {
			k_context.append_id(c_item_l, HP_OBJECT);
			return new Bag(k_graph, k_context);
		}
		// not exist
		else {
			throw 'handling not exists';
		}
	}
...
class Sources extends PathLeg {
	...
	cross(z_edge) {
		...
		if('string' === typeof z_edge) {
			...
			// search for word in predicates dict
			let i_item_p = search_fc_dict(k_graph.section_p, ab_word);
			if(i_item_p) {
				// append id to path
				k_context.append_id(i_item_p, HP_PREDICATE);
	...
	invert(z_edge) {
		...
		if('string' === typeof z_edge) {
			...
			if(i_item_p) {
				// append id to path
				k_context.append_id(i_item_p, HP_INVERSE_PREDICATE);
...
class Edge extends PathLeg {
	...
	literal(s_n3) {
		...
		let i_item_l = search_fc_dict(k_graph.section_l, ab_word, k_graph.range_o);
		if(i_item_l) {
			k_context.append_id(i_item_l, HP_OBJECT);
...
class InverseEdge extends PathLeg {
	...
	node(s_n3) {
		...
		if(i_item_s) {
			k_context.append_id(i_item_s, HP_SUBJECT);
			return new Source(k_graph, k_context);
		}
	}
