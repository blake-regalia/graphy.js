...
@ // 
	...
	style() {
		...
		// Vp
			...
			if(H_STOMP_ALL === hp_edge_type) {
				f_edges = function*(i_source) {
					@{each_edge()}
						yield [i_test_p, c_offset_data_p];
					@{end_each()}
				};
			}
			// ??
		...
		// Vp
			...
			// Vp[all]
			if(H_STOMP_ALL === hp_sink_type) {
				f_sinks = function*(i_source, c_offset_data_p) {
					@{each_object()}
						yield i_test_o;
					@{end_each()}
				};
		...
		for(let i_source of f_sources()) {
			...
			for(let [i_edge, c_offset_data_p] of f_edges(i_source)) {
				...
				for(let i_sink of f_sinks(i_source, c_offset_data_p)) {
	...
	_build_style(a_rows, h_source_row, i_hop) {
		let k_graph = this.graph;
		let k_context = this.context;
		...
		// each edge from pattern
		for(let [i_edge, c_offset_data_p] of f_edges(i_hop)) {
			// prep edge row to extend / copy-ref source row
			let h_edge_row = h_source_row;
			...
			// each sink from pattern
			for(let i_sink of f_sinks(i_hop, c_offset_data_p)) {
				// prep sink row to extend / copy-ref sink row
				let h_sink_row = h_edge_row;
