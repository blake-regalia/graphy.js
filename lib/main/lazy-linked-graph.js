...
@macro each_abc()
	// pull up c's data index
	let i_data_a_bc = a_idx_x_bc[c_offset_data_ab];
	let i_data_a_bc_end = a_idx_x_bc[c_offset_data_ab + 1];
	...
	o(i_object) {
		...
		// literal
			...
			try {
				...
				// word is utf-16le encoded
				if(!ab_literal[0]) k_literal = graphy.literal(decode_utf_16le(ab_literal.slice(1)));

				// word is utf-8 encoded
				else k_literal = graphy.literal(decode_utf_8(ab_literal));
			}
			catch(e) {
				debugger;
			}
