...
@include '../flavors/linker-macros.builder-js'
...
@macro write_typed_array(array, terminus)
	// @{array}
	d_transform.push(Buffer.from([@{array}.BYTES_PER_ELEMENT]), 'ascii');
	d_transform.push(Buffer.from(@{array}.buffer, @{array}.byteOffset, @{array}.byteLength), 'utf8');
	d_transform.push(Buffer.from([@{terminus}]), 'ascii');
@end
	...
	constructor(w_input, h_config) {
		...
		let h_parse_config = Object.assign(h_config.parse_options || {}, {
			...
			// finished scanning input
				...
				let n_data_p_size = c_distinct_sp + c_words_d + c_words_s;
				let a_data_p = @{mk_uint_array('n_data_p_size', 'c_words_p')};
				let i_data_p = 0;  // pointer for array above

				// prep data storage (compact triples structure) for objects
				let n_data_o_size = c_triples_total + c_distinct_sp;
				let n_objects = c_words_d + c_words_s + c_words_o + c_words_l;
				let a_data_o = @{mk_uint_array('n_data_o_size', 'n_objects')};
				...
				if(!w_input) {
					...
					@{write_typed_array('a_dict_d', 2)}
					@{write_typed_array('a_dict_s', 2)}
					@{write_typed_array('a_dict_o', 2)}
					@{write_typed_array('a_dict_l', 2)}
					@{write_typed_array('a_dict_p', 2)}

					// write each data section to output
					@{write_typed_array('a_data_p', 0)}
					@{write_typed_array('a_data_o', 0)}
				}
