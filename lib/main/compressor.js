...
@macro id_generator(name)
	// start index (0x00 = delimiter, 0x01 = prefix separator, 0x02 = absolute iri, 0x03 = blank node)
	let i_@{name}_id = 0x04;
	...
	function next_@{name}_id() {
		...
		if(0x10000 === i_@{name}_id) {
			...
			next_@{name}_id = function() {
				...
				return String.fromCharCode((i_@{name}_id >> 0x10) + 3) + String.fromCharCode(x_lo);
			};
		...
		return String.fromCharCode(i_@{name}_id++);
	}
@end
...
@macro compress_named_node(position)
	...
	if(m_compress_@{position}) {
		...
		// mark as uncompressed node for later eval
	...
	constructor(w_input, h_config) {
		...
		let h_parse_config = Object.assign(h_config.parse_options || {}, {
			...
			// finished scanning input
				...
				@{substitute_prefixes('h_nodes')}
				@{substitute_prefixes('h_predicates')}

				// go searching for all literals that need to be compressed
				for(let s_literal_id in h_literals) {
					// uncompressed datatype
					let m_literal = /\^[\u0000]([^"]+)"(.*)$/.exec(s_literal_id)
					if(m_literal) {
						let [, p_datatype_iri, s_content] = m_literal;

						// prep to replace literal id
						let s_new_literal_id;

						// compress iri (we can expect this will always work since it had to before)
						let [, p_compress_prefix, s_compress_suffix] = R_COMPRESS.exec(p_datatype_iri);

						// prefix iri is used elsewhere
						let s_prefix_id = h_prefix_lookup[p_compress_prefix];
						if(s_prefix_id) {
							s_new_literal_id = s_prefix_id+'\u0001'+s_compress_suffix;
						}
						// prefix never used anywhere else
						else {
							// search for an established prefix that can support this iri
							...
							// found one!
							if(p_best_prefix_iri) {
								s_new_literal_id = h_prefix_lookup[p_best_prefix_iri]+'\u0001'+p_datatype_iri.slice(n_best_prefix_len+1);
							}
							// no usable prefix was found; use full iri
							else {
								s_new_literal_id = '\u0002'+p_datatype_iri;
							}
						}

						// change this entry's key
						let w_node = h_literals[s_literal_id];
						delete h_literals[s_literal_id];
						h_literals[s_new_literal_id] = w_node;
					}
				}
