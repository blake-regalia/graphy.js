...
// 
const LinkedGraph = require('./linked-graph.js');
		...
		let h_parse_config = Object.assign(h_config.parse_options || {}, {
			...
			data(h_triple) {
				...
				if(h_object.isLiteral) {
					...
					// build literal's word
					let s_literal_id = h_object.value.replace(/"/g, '\\"')+'"';
					...
					if(h_object.hasOwnProperty('datatype')) {
						...
						s_literal_id += '^'+s_datatype_id;
					}
					// literal has language tag
					else if(h_object.hasOwnProperty('language')) {
						// add to literal's word
						s_literal_id += '@'+h_object.language;
					}
			...
			// finished scanning input
				...
				for(let i_fn in h_free_nodes) {
					...
					for(let i_dict_p in h_pairs) {
						...
						for(let i_ in a_object_literals) {
							...
							a_data_o[i_data_o++] = c_words_d + c_words_o + a_object_literals[i_] + 1;
						}
				...
				if(!w_input && d_transform && 'function' === typeof d_transform.push) {
					...
					@{write_typed_array('a_data_p', true)}
					@{write_typed_array('a_data_o', true)}
				}
