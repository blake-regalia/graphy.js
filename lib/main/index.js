			...
			h_properties['$type'] = {
				value(s_namespace) {
					...
					if(1 < a_tersable_types.length) {
						...
						local.warn(`more than one triple share the same predicate "${n3_shorten(P_IRI_RDF+'type', h_prefixes)}" with subject "${h_node.id}". By using '.$type()', you are accessing any one of these triples arbitrarily`);
					}
				...
				h_properties[s_access_name] = {
					...
					get() {
						...
						if(a_nodes.length > 1) {
							...
							// issue warning about using first triple
							local.warn(`more than one triple share the same predicate "${n3_shorten(s_key, h_prefixes)}" with subject "${h_node.id}". By using '.${s_access_name}', you are accessing any one of these triples arbitrarily`);
						}
