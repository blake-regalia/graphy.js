...
* class:
**/
const local = classer('Graphy', (h_jsonld) => {
	...
	**/
			...
			if(m_prefix) {
				...
				if(h_context[m_prefix[1]]) {
					...
					// primary context is a json-ld context
						...
						// only using primary context as prefix
						return {
							id: s_primary_context_id+m_prefix[2],
						};
	...
	const expand = (s_iri) => {
		...
		if('string' !== typeof z_context_ref) {
			local.fail('did not find a prefix mapping for "'+m_prefix[1]+'" from "'+s_iri+'"; instead found: '+arginfo(z_context_ref));
		}
	...
	**/
	...
	const wrap_node = (z_object, h_parent_context, s_key='') => {

		//
		let b_ignore_context = s_key.includes(':');
		...
		if('object' === s_type && !Array.isArray(z_object)) {
			...
			if('string' === typeof z_object['@id']) {
				...
				// local.good('** resolving:\n'+arginfo(z_object)); 
				return node_iri(z_object['@id']);
			}
			...
			for(let s_property in h_object) {
				...
				if('@' === s_property[0]) {
					...
					// json-ld is giving collection
					else if('@list' === s_property) {

						// map each list item to wrapper; return list as single element in array
						return [{
							type: 'collection',
							list: z_object['@list'].map((z_item) => {
		...
		// ref context type
		let s_context_type = h_node_context.type || h_parent_context.type;
		...
		if('@id' === s_context_type) {
			...
			if('string' === s_type) {
				...
				// context identifies an iri
				return node_iri(z_object);
			}
			// object type is a javascript object
				...
				if(Array.isArray(z_object)) {
					...
					return z_object.map((z_item) => {
						let a_nodes = wrap_node(z_item, h_parent_context);
	...
	const cover_iri = (h_iri, p_namespace) => {
		...
		let h_properties = {
			...
			// full SPARQL/TTL-ready string representation of iri
			'@full': {
				value: '<'+h_iri.iri+'>',
			},
		};
	...
	const cover_literal = (h_literal, p_namespace) => {
		...
		let h_properties = {
			...
			// generate terse string representation of this node (with auto-prefix)
			$terse: {
				value: Object.defineProperties(function() {
					return s_quoted_value+'^^'+shorten(s_datatype);
				}, {

					// auto-prefix terse datatype
					datatype: {
						value: function() {
							return shorten(s_datatype);
						},
					},
				})
			},

			// full SPARQL/TTL-ready string representation of literal
			'@full': {
	...
	const cover_collection = (h_collection, p_namespace) => {
		...
		let h_properties = {
			...
			$: {
				value: function(s_namespace) {
					return cover_collection(h_collection, expand(s_namespace));
				},
			},
			...
			'@full': {
				get: ()=> {
					return '['
						+'<'+P_IRI_RDF+'first> '+a_list_covered[0]['@full']+';'
						+'<'+P_IRI_RDF+'rest> ('+a_list_covered.slice(1).map(h => h['@full']).join(' ')+')'
						+']';
				},
			},
		...
		for(let s_key in h_node) {
			...
			// ld predicate
				...
				if(p_predicate.startsWith(p_namespace)) {
					...
					let a_nodes = wrap_node(z_value, h_node_context, s_key);
