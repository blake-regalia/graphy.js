		...
		let h_properties = {
			...
			$terse: {
				value() {
					...
					// node is blanknode
					if(b_blanknode) return h_node['@id'];

					// otherwise, shorten iri
					return n3_shorten(h_node['@id']);
				},
			},
			...
			'@full': {
				get() {
					return () => {
						// not yet implemented
						throw '@full-ing a node is not yet implemented';
					};
				},
			},
