...
function NamedNode(s_iri) {
	...
	Object.create(GenericTerm.prototype), {
		...
		terse(h_prefixes={}) {
			let sct = concise(this.value, h_prefixes);
			if('>' === sct[0]) return '<'+this.slice(1)+'>';
			return sct;
		},
		toObject() {
			return {
...
function Literal(s_value, z_datatype_or_lang) {
	...
	Object.create(GenericTerm.prototype), {
		...
		concise() {
			return concise(this.datatype.value);
		},
		terse(h_prefixes={}) {
			// concise datatype node
			let sct_datatype = concise(this.datatype.value, h_prefixes);

			// turn into terse
			let st_datatype = '>' === sct_datatype[0]? '<'+sct_datatype+'>': sct_datatype;
...
class QuadSet {
	...
	hash_quad(h_quad, h_hashed, a_visited) {
		...
		return h_quad.graph.concise()+'\n'
			+(h_subject.isBlankNode
				? '_'+this.hash_blank_node(h_subject.concise(), h_hashed, a_visited)
				: h_subject.concise()+'')+'\n'
			+h_quad.predicate.concise()+'\n'
			+(h_object.isBlankNode
				? '_'+this.hash_blank_node(h_object.concise(), h_hashed, a_visited)
				: h_object.concise())+'\n';
	}
	...
	canonicalize() {
		...
		for(let p_blank in h_root_blanks) {
			...
			h_root_blanks[p_blank].forEach((h_quad) => {
				...
				let h_triples = h_tree[h_quad.graph.concise()];
		...
		for(let p_blank in h_leaf_blanks) {
			...
			h_leaf_blanks[p_blank].forEach((h_quad) => {
				...
				// update tree
				let as_objects = h_tree[h_quad.graph.concise()][h_quad.subject.concise()][h_quad.predicate.concise()];
