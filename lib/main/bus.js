...
const I_MSMIL               = 0x11;  // multiple strictly monotonically increasing lists
const I_CSMII               = 0x12;  // complementary strictly monotonically increasing indexes
const I_ECSMII              = 0x13;  // empty "         "
...
const AB_MSMIL              = Buffer.from([I_MSMIL]);
const AB_CSMII              = Buffer.from([I_CSMII]);
const AB_ECSMII             = Buffer.from([I_ECSMII]);
...
const front_coded_sum = (ab_word, ab_diff) => {
	let [n_shared, i_read] = decode_pint(ab_diff, 0);
...
const H_ENCODERS = {
	...
	null_terminated_fc_dict: (ab_dict, a_chunk, at_len, ...a_args) => {
		...
		for(let i_word=0; i_word<n_words;) {
			...
			while(i_word < i_max) {
				...
				let n_shared; [n_shared, i_read] = decode_pint(ab_dict, i_read);
	...
	null_terminated_fc_dict_compressed: (ab_dict, a_chunk, at_len, ...a_args) => {
		// count how many bytes we'll be needing
		let n_words = at_len.length;
		let c_bytes = ab_dict.length + n_words;
...
// debugger;
		...
		for(let i_word=0; i_word<n_words;) {
			let n_length = at_len[i_word++];
			let ab_word_new = ab_dict.slice(i_read, i_read+n_length);
			let ab_coded = i_word === 1? ab_word_new: front_coded_diff(ab_word, ab_word_new);
			...
			while(i_word < i_max) {
				...
				let n_shared; [n_shared, i_read] = decode_pint(ab_dict, i_read);
				let n_unshared = n_length - n_shared;
				ab_coded = ab_dict.slice(i_prior_shared, i_read+n_unshared);
	...
	multiple_strictly_monotonically_increasing_lists(at_data, a_chunk, at_idx) {
		...
		a_chunk.push(AB_MSMIL);
		a_chunk.push(ab_huffman);

		return 1 + ab_huffman.length;
	},

	nested_multiple_strictly_monotonically_increasing_lists(at_data, a_chunk, a_idx_super) {
...
debugger;
	...
	complementary_strictly_monotonically_increasing_index(at_idx, a_chunk, at_data) {
		let n_idxs = at_idx.length;
		let a_reduced = [];
...
const H_TYPES = {
	...
	CSMII: 'complementary_strictly_monotonically_increasing_index',
	MSMIL: 'multiple_strictly_monotonically_increasing_lists',
	NMSMIL: 'nested_multiple_strictly_monotonically_increasing_lists',
	NCSMII: 'nested_complementary_strictly_monotonically_increasing_index',
};
...
const decode_sorted_list = (a, i) => {
	...
	let n_items; [n_items, i] = decode_pint(a, i);

	// top of each byte in item value
	let x_top; [x_top, i] = decode_pint(a, i);
...
const decode_sorted_list_frag = (a, i, n) => {
	...
	// length of output list
	let a_items_frag = decode_pint_frag(a, i, n);
	if(!a_items_frag) return false;
	let n_items; [n_items, i] = a_items_frag;

	// top of each byte in item value
	let a_top_frag = decode_pint_frag(a, i, n);
	if(!a_top_frag) return false;
	let x_top; [x_top, i] = a_top_frag;
...
const decode = (a, i) => {
	...
	switch(i_type) {
		...
		// negative int
		case I_NINT: {
			let x_value; [x_value, i] = decode_pint(a, i);
			return [-x_value, i];
		}
		...
		// utf-8 string
		case I_UTF_8_STRING: {
			let n_bytes; [n_bytes, i] = decode_pint(a, i);
		...
		case I_HASH: {
			let h_hash = {};
			let n_pairs; [n_pairs, i] = decode_pint(a, i);

			for(let i_pair=0; i_pair<n_pairs; i_pair++) {
				let n_key_length; [n_key_length, i] = decode_pint(a, i);

				let s_key = a.slice(i, i+n_key_length).toString('utf8');
				i += n_key_length;

				let w_value; [w_value, i] = decode(a, i);
		...
		case I_ARRAY: {
			let a_items = [];
			let n_size; [n_size, i] = decode_pint(a, i);

			for(let i_item=0; i_item<n_size; i_item++) {
				let at_item; [at_item, i] = decode(a, i);
		...
		// buffer
		case I_BUFFER: {
			let c_bytes; [c_bytes, i] = decode_pint(a, i);
			return [a.slice(i, i+c_bytes), i+c_bytes];
		}

		// Uint8Array
		case I_UINT_8_ARRAY: {
			let c_bytes; [c_bytes, i] = decode_pint(a, i);
			return [new Uint8Array(a.slice(i, i+c_bytes)), i+c_bytes];
		}

		// otherwise
		default: {
			let n_length; [n_length, i] = decode_pint(a, i);
...
const decode_frag = (a, i, n) => {
	while(i < n) {
		...
		switch(i_type) {
			...
			// negative int
			case I_NINT: {
				let a_nint = decode_pint(a, i, n);
				if(!a_nint) return false;
				let x_value; [x_value, i] = a_nint;
				return [-x_value, i];
			}
			...
			// utf-8 string
			case I_UTF_8_STRING: {
				let a_utf8 = decode_pint_frag(a, i, n);
				if(!a_utf8) return false;
				let n_bytes; [n_bytes, i] = a_utf8;
				return [a.slice(i, i+n_bytes).toString('utf-8'), i+n_bytes];
			}

			// utf-16 string
			case I_UTF_16_STRING: {
				let a_utf16 = decode_pint_frag(a, i, n);
				if(!a_utf16) return false;
				let n_bytes; [n_bytes, i] = a_utf16;
				return [a.slice(i, i+n_bytes).toString('utf-16le'), i+n_bytes];
			}

			// hash
			case I_HASH: {
				let a_frag = decode_pint_frag(a, i, n);
				if(!a_frag) return false;
				let n_pairs; [n_pairs, i] = a_frag;
				let h_hash = {};

				for(let i_pair=0; i_pair<n_pairs; i_pair++) {
					let a_key_frag = decode_pint_frag(a, i, n);
					if(!a_key_frag) return false;
					let n_key_length; [n_key_length, i] = a_key_frag;

					let s_key = a.slice(i, i+n_key_length).toString('utf8');
					i += n_key_length;

					let a_value_frag = decode_frag(a, i, n);
					if(!a_value_frag) return false;
					let w_value; [w_value, i] = a_value_frag;
			...
			case I_ARRAY: {
				let a_frag = decode_pint_frag(a, i, n);
				if(!a_frag) return false;
				let n_size; [n_size, i] = a_frag;

				let a_items = [];

				for(let i_item=0; i_item<n_size; i_item++) {
					let a_item_frag = decode_frag(a, i, n);
					if(!a_item_frag) return false;
					let at_item; [at_item, i] = a_item_frag;
			...
			// otherwise
			default: {
				let a_length_frag = decode_pint_frag(a, i, n);
				if(!a_length_frag) return false;
				let n_length; [n_length, i] = a_length_frag;
...
class Collector extends stream.Writable {
	constructor(z_receiever, i_consumer, h_stream_opt={}) {
		...
		this.on('finish', () => {
			...
			let [w_result, i] = decode(a, 0);
			a_results.push(w_result);
			let n = a.length;
			while(i < n) {
				[w_result, i] = decode(a, i);
				a_results.push(w_result);
			}
