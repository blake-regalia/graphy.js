...
const F_SORT_BUFFER = (h_a, h_b) => {
	let ab_a = h_a.buffer;
	let ab_b = h_b.buffer;
	let n = Math.min(ab_a.length, ab_b.length);
	for(let i=0; i<n; i++) {
		let x_a = ab_a[i];
		let x_b = ab_b[i];
	...
	function next_@{name}_id() {
		...
		if(0x10000 === i_@{name}_id) {
			...
			next_@{name}_id = function() {
				...
				while(x_lo < 2) x_lo = (i_@{name}_id++) % 0xffff;

				// start hi bits off at 4 to avoid collision w/ 0, 1, 2, and 3
				return Buffer.from([(i_@{name}_id >> 0x10) + 3, x_lo@{separator? ', '+separator: ''}]);
			};
		...
		return Buffer.from([i_@{name}_id++@{separator? ', '+separator: ''}]);
	}
@end
...
@macro assign_ids(type, offset, track, short)
	...
	for(let i_@{type}=0; i_@{type}<n_@{type}s; i_@{type}++) {
		...
		@end
		@if short
			h_thing.index = X_SHORT_LIST | (@{offset? offset+' + ': ''}i_@{type} + 1);
		@else
...
@macro data_from_list(src)
	...
	for(let i_@{src}=0; i_@{src}<n_@{src}s; i_@{src}++) {
		...
		// starting position
		a_s_po_list.push(a_data_s_po.length);
		...
		for(let i_pair=0; i_pair<n_pairs; i_pair++) {
			...
			for(let i_node=0; i_node<n_nodes; i_node++) {
				a_data_s_po.push(a_nodes[i_node].index);
			}
			...
			for(let i_literal=0; i_literal<n_literals; i_literal++) {
				a_data_s_po.push(a_literals[i_literal].index);
			}
		...
		//
		let n_s_po_list = a_s_po_list.length;
		let i_max_ptr = a_s_po_list[n_s_po_list - 1];
		let at_s_po_list = @{mk_uint_array('n_s_po_list', 'i_max_ptr')}
		at_s_po_list.set(a_s_po_list);

		// push s_po list to idx
		a_idx_s_po.push(at_s_po_list);

		// reset s_po list variable
		a_s_po_list = [];

		// sp index
		a_idx_sp.push(a_data_sp.length);
	}
@end
		...
		async.series([
			...
			(fk_task) => fs.createReadStream(p_file).pipe(h_config.parse({
				...
				data(h_triple) {
					...
					if(!h_object_node) {
						...
						if(h_object.isLiteral) {
							...
							// save literal
							h_literals[s_object_id] = h_object_node;
						}
						// object is node
						else {
							// ... a named node
					...
					// object exists as node
					else if(!h_object.isLiteral) {
					...
					// object is literal
					if(h_object.isLiteral) {
						h_adj_lists.l.push(h_object_node);
					}
					// object is node
					else {
				...
				// finished scanning input
					...
					f_progress(1);
					@{assign_ids('predicate', false, 'word', true)}
					f_progress(1);
					...
					// serialize data lists & idx list
					let at_data_sp = @{mk_uint_array('a_data_sp.length', 'n_predicates << 1')}  // reserve a bit for short list mask
					at_data_sp.set(a_data_sp);
					let i_max_object = n_duals + n_objects + n_literals;
					let at_data_s_po = @{mk_uint_array('a_data_s_po.length', 'i_max_object')}
					at_data_s_po.set(a_data_s_po);
					let n_idx_sp = a_idx_sp.length;
					let i_max_sp = a_idx_sp[n_idx_sp-1];
					let at_idx_sp = @{mk_uint_array('n_idx_sp', 'i_max_sp')}
					at_idx_sp.set(a_idx_sp);
