...
@include '../store/store.jmacs'

@set range_counter 0
@set range_name ''

const bus = require('./bus');
const triples = require('../store/triples');
	...
	constructor(p_file, h_config) {
		...
		async.series([
			...
			(fk_task) => h_config.parse(fs.createReadStream(p_file), {
				...
				// if any prefix changes, then the final map is not valid throughout the entire document
				prefix_change() {
				...
				// finished scanning input
					...
					for(let p_prefix_iri in h_prefix_candidates) {
						// prefix used more than NEVER -<strikethru>-once-</>-
						if(h_prefix_candidates[p_prefix_iri] > 0) {  // > 1
			...
			(fk_task) => fs.createReadStream(p_file).pipe(h_config.parse({
				...
				data(h_triple) {
					...
					let s_subject_id = h_subject.verbose();
					...
					let s_predicate_id = h_predicate.verbose();
					...
					let h_adj_lists = h_pairs[i_predicate_temp] = h_pairs[i_predicate_temp]
						|| (c_distinct_sp++, {l:[], n:[], lh:{}, nh:{}});


					// canonicalize object
					let s_object_id = h_object.verbose();
				...
				// finished scanning input
					...
					let at_adj_sp_o = @{mk_uint_array('c_triples', 'i_max_object')};
					...
					Object.assign(k_graph, {
						...
						triples_spo: triples.contiguous_adjacency_list_triples(k_graph, 's', 'p', 'o', at_idx_s_p, at_adj_s_p, at_idx_sp_o, at_adj_sp_o),
