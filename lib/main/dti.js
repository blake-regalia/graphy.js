...
class Consumer extends stream.Writable {
	...
	_write(db_chunk, s_encoding, f_okay) {
		...
		while(n_chunk_length - i_reader >= n_bytes_needed) {
			...
			// hash (all utf8)
				...
				while(i_reader <= i_buffer_end) {
					...
					// extract key and value (separator char = 0x01)
					let i_split = db_chunk.indexOf(1, i_reader);
		...
		if(this.config.progress) {
			this.config.progress(i_reader, h_section? h_section.key: 'finished');
		}
