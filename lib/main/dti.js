...
const D_UTF_8_TEXT_ENCODER = B_ENV_NODE? null: new TextEncoder('utf-8');

const encode_utf_8 = B_ENV_NODE
	? (s_chunk) => {
...
const D_UTF_8_TEXT_DECODER = B_ENV_NODE? null: new TextDecoder('utf-8');

const decode_utf_8 = B_ENV_NODE
	? (ab_chunk) => {
...
class Consumer extends stream.Writable {
	...
	_write(zb_chunk, s_encoding, f_okay) {
		...
		if('string' === typeof zb_chunk) {
			...
			a = Buffer.from(zb_chunk, s_encoding);
		}

		// augment chunk with previous leftovers
		if(this.previous) {
			a = Buffer.concat([this.previous, a], this.previous.length + a.length);
			this.previous = null;
		}
		...
		while(n - i > 0) {
			...
			// start of section
				...
				// start new section
				h_section = Object.assign({}, H_SECTIONS[x_type]);
		...
		if(i < n) {
			// store remainder
			this.previous = Buffer.from(a.slice(i));
		}

		if(this.config.progress) {
			this.config.progress(i, h_section? h_section.key: 'finished');
		}
...
class Serializer {
	constructor(ds_output, h_config, fk_writer) {
		...
		ds_output.write(Buffer.from([0x80 | N_VERSION]));
	...
	write_bytes(a_bytes, s_section) {
		return {
			drained: this.output.write(Buffer.from(a_bytes)),
			section: s_section,
		};
	}
	...
	write_array(x_type, x_phase, s_key) {
		...
		let b_drained = ds_output.write(Buffer.from([
		...
		// write data
		return {
			drained: b_drained && ds_output.write(Buffer.from(at_array.buffer, at_array.byteOffset, at_array.byteLength)),
			section: s_key,
		};
	}
...
debugger;
		...
		for(let i_arr=0; i_arr<n_arr; i_arr++) {
			...
			b_drained &= ds_output.write(Buffer.from([
				// bytes per element
				at_array.BYTES_PER_ELEMENT,
			...
			// write data
			b_drained &= ds_output.write(Buffer.from(at_array.buffer, at_array.byteOffset, at_array.byteLength));
		}
