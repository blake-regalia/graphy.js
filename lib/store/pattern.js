...
@macro iterate_b(no_row)
	// iterate b
	@if no_row
		for(let {id: i_b, offset: c_offset_data_ab} of f_b(i_a)) {
	@else
...
@macro mk_generator_b(rows)
	generate_b_@{rows? 'rows': 'ids'}(h_entity, s_term, s_key) {
		let k_graph = this.graph;
			...
			if(HP_RANGE_ALL === hp_entity_range) {
				...
				return function*(i_test_a, h_row_a) {
					@{each_ab()}
						...
						if(f_filter && !f_filter(k_graph[s_term](i_test_b))) continue;
						...
						@if rows
							...
							if(b_store) {
								...
								// store saved
								if(s_save) h_row_b[s_save] = k_instance.load(i_test_b);
							}
...
class Selection {
	...
	next_triple_pattern(h_head, k_pattern) {
		...
		// prep term positions
		let s_term_a = '', s_term_b = '', s_term_c = '';
...
class Entrance {
	...
	path() {
		return new EmptyPath(this.graph);
	}
}
...
class EmptyPath {
	...
	from(h_from) {
		...
		if(h_from.node) {
			...
			// carry on
			let a_root = [h_node];
			return new PartialPath(k_graph, a_root, a_root);
		}
...
class PartialPath {
	constructor(k_graph, a_root, a_from) {
		this.graph = k_graph;
		this.root = a_root;
		this.from = a_from;
	}
	...
	find(h_src, h_dests, n_min=0, n_max=Infinity, h_visited={}, n_depth=1) {
		// exceeded maximum path length
		if(n_depth > n_max) return;
...
// debugger;
	...
	to(h_to) {
		...
		if(h_to.source) {
			...
			if('function' === typeof z_source) {
				...
				//
				return new PartialPath(k_graph, this.root, a_dests);
			}
		}
	...
	exit() {
		return new TrailHead(this.graph, this.root);
	}
}

class TrailHead {
	constructor(k_graph, a_srcs) {
		this.graph = k_graph;
		this.srcs = a_srcs;
	}
	...
	roots(fk_root) {
		...
		@{each('src', 'h')}
			let h_root_vertex = k_graph.v(h_src.id, h_src.type);
			fk_root(new PathExplorer(k_graph, h_src.vectors));
		@{end_each()}
	}
