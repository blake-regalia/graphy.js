...
@macro iterate_b(b)
	// iterate b
	for(let {id: i_@{b}, row: h_row_b, offset: c_offset_data_@{a}@{b}} of f_@{b}(i_@{a}, h_row_a)) {
@end
...
class Selection {
	...
	*results() {
	...
	iterate_c_ids(h_entity, s_term, s_key) {
		let k_graph = this.graph;
...
class EmptyPath {
	...
	from(h_from) {
		...
		if('string' === typeof z_target) {
			k_graph.encode_n3_to_word(z_target);
		}

		k_pattern.append_id();

		return new PartialPath(k_graph, k_pattern);
	}
}
...
class PartialPath {
	...
	find(h_src, i_dest, h_visited, n_depth) {
		...
		let a_scan = [];
		if(i_src < k_graph.range_d || hp_type === k_graph.TYPE_SUBJECT) {
	...
	to() {
		...
		let h_a = find_segments(i_a, x_type_a, i_b, a_paths);
		let h_b = find_segments(i_b, x_type_b, i_a, a_paths);
		
		intersect(h_a.left, h_b.left, h_b.right);
		intersect(h_a.right, h_b.left, h_b.right);
	}
}
