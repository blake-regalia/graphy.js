	...
	*results() {
	...
	iterate_a(h_entity, s_term) {
		...
		if(h_entity.id) {
			...
			if(h_data) {
				...
				if(!h_data.plugin.check(h_data.action, i_entity)) {
					// empty iterator
					return function*() {};
				}
			}
		...
		// K*[+]
			...
			if(h_data) {
				...
				@{each('entity_id', 'i', 'entity')}
					...
					if(k_plugin.check(f_action, i_entity)) {
						// preserve entity in set
						a_entity_ids_cleaned.push(i_entity);
					}
				@{end_each()}
	...
	iterate_c(h_entity, s_term, s_key) {
		...
		if(h_entity.id) {
			...
			if(h_data) {
				// apply plugin handler; action dissaproves of this entity
				if(!h_data.plugin.scoped(h_data.action, i_entity)) {
					// empty iterator
					return function*() {};
				}
			}
...
class GraphPatternEntity {
	...
	data(s_register_id, f_action) {
		...
		let k_plugin = k_graph.data_plugins[s_register_id];
		if(k_plugin) {
			// save action
			k_pattern.end().data = {
				plugin: k_plugin,
...
@macro literal()
	literal(s_content, z_datatype_or_lang) {
		...
		return new EmptyLiteral(k_graph, null, {
			literal: {
				content: s_content,
