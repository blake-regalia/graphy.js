...
class FrontCodedNullVByteSection {
	...
	produce(i_term) {
		...
		if(i_word < i_key) {
			...
			do {
				...
				if(n_share >= 0x80) {
					let h=bus.decode_pint(ab_dict, i_dict); n_share=h.v; i_dict=h.i;
				}
				else i_dict++;
				...
				if(nl_word >= 0x80) {
					let h=bus.decode_pint(ab_dict, i_dict); nl_word=h.v; i_dict=h.i;
				}
				else i_dict++;
	...
	search_fc_block(ab_word, i_block, b_skip_head) {
		...
		for(let i_block_idx=1; i_block_idx<n_block_size; i_block_idx++) {
			...
			if(n_share >= 0x80) {
				let h=bus.decode_pint(ab_dict, i_dict); nl_share=h.v; i_dict=h.i;
			}
			else i_dict++;
			...
			if(nl_code >= 0x80) {
				let h=bus.decode_pint(ab_dict, i_dict); nl_code=h.v; i_dict=h.i;
			}
			else i_dict++;
