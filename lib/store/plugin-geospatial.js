...
class Multipoint extends GeometricFeature {
	constructor(i_id, a_points) {
		...
		let i_write = 0;
		let x_l=Infinity, x_b=Infinity, x_r=-Infinity, x_t=-Infinity;

		@{each('point', 'h')}
			let x_x=h_point.x, x_y=h_point.y;
...
const ring_contains_point(a_ring, x_x, x_y, b_ignore_boundary) {
	...
	let {x:x_0x, y:x_0y} = a_ring[nl_ring-1];
	for(let i_point=0; i_point<nl_ring-1; i_point++) {
		let {x:x_1x, y:x_1y} = a_ring[i_point];
