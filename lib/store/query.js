...
@macro hop()
	hop(s_n3) {
		...
		// no such hop in set
		return new EmptyHopFix(k_graph, null, {n3: s_n3});
	}
@end
...
@macro subject()
	subject(s_n3) {
		...
		return new EmptySubjectFix(k_graph, null, {n3: s_n3});
	}
@end
...
@macro object()
	object(s_n3) {
		...
		return new EmptyObjectFix(k_graph, null, {n3: s_n3});
	}
@end
...
@macro literal()
	literal(s_tt_literal) {
		...
		// start of content
		let i_content = s_tt_literal.indexOf('"');
		...
		// not a literal
			...
			throw `invliad tt_string for literal: ${s_tt_literal}`;
		}
		// non-plain literal
		else {
			let s_chr = s_tt_literal[0];

			// languaged literal
			if('@' === s_chr) {
				let ab_lang = encode_utf_8(s_tt_literal.slice(0, i_content));
			...
			// datatyped literal
				...
				let ab_datatype = k_graph.encode_n3_to_word(s_tt_literal.slice(1, i_content));
			...
			// invalid tt_string
				...
				throw `invalid tt_string for literal: ${s_tt_literal}`;
			}
		}
		...
		// encode content
		let s_content = s_tt_literal.slice(i_content+1);
