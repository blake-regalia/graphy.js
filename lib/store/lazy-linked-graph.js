...
class LinkedGraph {
	...
	*a_bc(i_a, a_data_ab, a_idx_ab, a_data_a_bc, a_idx_a_bc) {
		// search ab's adjacency list for predicate
		let c_offset_ab = 0;
		let i_data_ab_start = a_idx_ab[i_a - 1];
		let a_idx_x_bc = a_idx_a_bc[i_a - 1];
		let c_offset_ab_end = a_idx_x_bc.length - 1;

		// each predicate in subject's ab adjacency list
		do {
			// pull up b's id
			let i_b = a_data_ab[i_data_ab_start + c_offset_ab];

			//
			for(let i_c of this.ab_c(c_offset_ab, a_data_a_bc, a_idx_x_bc)) {
				yield {
					b: i_b,
					c: i_c,
				};
	...
	*ab_c(c_offset_ab, a_data_a_bc, a_idx_x_bc) {
		...
		do {
			// yield c's id
			yield a_data_a_bc[i_data_a_bc];
		} while(++i_data_a_bc !== i_data_a_bc_end);
	}

	*s_po(i_s) {
		if(!this.data_sp) throw 'SPO index not built';
		yield* this.a_bc(i_s, this.data_sp, this.idx_sp, this.data_s_po, this.idx_s_po);
	}

	*p_os(i_p) {
		if(!this.data_po) throw 'POS index not built';
		yield* this.a_bc(i_p, this.data_po, this.idx_po, this.data_p_os, this.idx_p_os);
	}

	*o_sp(i_o) {
		if(!this.data_os) throw 'OSP index not built';
		yield* this.a_bc(i_o, this.data_os, this.idx_os, this.data_o_sp, this.idx_o_sp);
	}
