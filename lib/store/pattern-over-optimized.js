...
@macro iterate_a(a, mark, save)
	...
	for(let i_@{a} of f_@{a}()) {
		@if mark || save
			...
			@if save
				// store saved
				h_row_a[s_save_a] = k_data_instance_a.load(i_@{a});
			@end
		@else
			let h_row_a = h_row__;
		@end
...
@macro iterate_c(c, mark, save)
	...
	for(let i_@{c} of f_@{c}(i_@{a}, c_offset_data_@{a}@{b})) {
		@if mark || save
			...
			@if save
				// store saved
				h_row_c[s_save_c] = k_data_instance_c.load(i_@{c});
			@end
		@else
			let h_row_c = h_row_b;
		@end
...
@macro mk_iterators(a, b, c)
	...
	if(s_save_a) k_data_instance_a = h_@{a}.data.plugin.instance;
	if(s_save_b) k_data_instance_b = h_@{b}.data.plugin.instance;
	if(s_save_c) k_data_instance_c = h_@{c}.data.plugin.instance;

	let b_extend_a = s_mark_@{a} || s_save_a;
	let b_extend_b = s_mark_@{b} || s_save_b;
	let b_extend_c = s_mark_@{c} || s_save_c;
@end


@macro mk_proceed_inner(a, b, c, yield)
	// ref head(s)
...
@macro mk_proceed_eval(a, b, c, yield)
	...
	if(!h_@{a}.range && !h_@{b}.range && h_@{c}.range  && k_pattern.length >= 2 && !k_pattern.peek(0).range && !k_pattern.peek(1).range) {
		...
		if(s_mark_@{a}) {
			if(s_save_a) {
				@{iterate_a(a, true, true)}
					if(s_mark_@{b}) {
						if(s_save_b) {
							@{iterate_b(b, true, true)}
								if(s_mark_@{c}) {
									if(s_save_c) {
										@{iterate_c(c, true, true)}
											@{mk_proceed_inner(a, b, c, yield)}
										@{end_iterate()}
									}
									else {
										@{iterate_c(c, true, false)}
											@{mk_proceed_inner(a, b, c, yield)}
										@{end_iterate()}
									}
								}
								// no mark c
									...
									if(s_save_c) {
										@{iterate_c(c, false, true)}
											@{mk_proceed_inner(a, b, c, yield)}
										@{end_iterate()}
									}
									else {
										@{iterate_c(c, false, false)}
											@{mk_proceed_inner(a, b, c, yield)}
										@{end_iterate()}
									}
								}
