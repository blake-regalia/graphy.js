...
const BunchMethods = {
	...
	// fetch all terms' values in this bunch as an array
	values() {
		return this.terms.map((h_term) => {
			return h_term.value;
		});
	},
...
function BunchOfLiterals(){

}
BunchOfLiterals.prototype = Object.assign(Object.create(Bunch.prototype), {

	filter(z_literal_filter) {
...
class Network {
	...
	constructor(h_graph, h_prefixes) {
		...
		**/
		...
		// filter literals
		function BunchOfLiterals(z_literal_filter) {
			// user wants to filter literal by language or datatype
			if('string' === typeof z_literal_filter) {
				// by language tag
				if('@' === z_literal_filter[0]) {
				...
				// by datatype
					...
					// resolve datatype iri
					let p_datatype = resolve(z_literal_filter);
		...
		const mk_Bunch = function(f_bunch_type, a_terms) {
			// operator
		...
		class Knob {
			constructor(h_graph, p_uri) {
				...
				Object.assign(this, {
					...
					value: p_uri,
					links: h_pairs,
					at: Object.setPrototypeOf(KnobLink.bind(h_context), PrefixNamespace),
				});
			}
			...
			// 		value: Object.create(PrefixNamespace, {
			// 			$: {
			// 				value: this.value,
			// 			},
			// 		}),
		...
		class Bag {
			constructor(a_objects) {
				Object.assign(this, {
					terms: a_objects || [],
				});
			}

			// filter elements in bag to get a bunch of literals
			get literals() {
				// materialize on instance
				Object.defineProperty(this, 'literals', {
					value: mk_Bunch(BunchOfLiterals, this.terms.filter((h_term) => {
		...
		**/
		...
		for(let i_node=0; i_node<a_nodes.length; i_node++) {
			h_entry_nodes[a_nodes[i_node]] = 1;
		}
	...
	// enter the network at the given node
	enter(z_node) {
		if('string' === typeof z_node) {
	...
	// 	for(let s_subject_id in h_entry_nodes) {
	// 		// ref 
	// 		h_entry_nodes[s_subject_id]
	// 	}
