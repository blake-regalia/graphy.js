...
export type Router<
	Direction extends string=string,
	YieldType=unknown,
> = {
	node?: (ds_stream: Direction extends `read:${string}`
		? NodeReadable
		: (Direction extends `write:${string}`
			? NodeWritable
			: NodeReadable | NodeWritable)) => void;

	whatwg?: (ds_stream: Direction extends `read:${string}`
		? ReadableStream
		: (Direction extends `write:${string}`
			? WritableStream
			: ReadableStream | WritableStream),
		// y_reader_writer?: Direction extends 'read'
		// ? ReadableStreamGenericReader
		// : (Direction extends 'write'
		// 	? WritableStreamDefaultWriter
			y_reader_writer?: ReadableStreamGenericReader | WritableStreamDefaultWriter) => void;

	function?: (f_write: Direction extends 'read:string'
		? ReadableFunction
		: (Direction extends 'read:object'
			? () => YieldType | Promise<YieldType>
			: (Direction extends 'write:string'
				? WritableFunction
				: ReadableFunction | WritableFunction))) => void;
...
>(z_stream: Direction extends `read:${string}`
	? ObjectReadable
	: (Direction extends `write:${string}`
		? StringWritable
		: ReadeableOrWritable), h_router: Router<Direction, ObjectType>) {
	// truthy
	if(z_stream) {
		// node.js stream
		...
		// simple callback optionally returning a promise
			...
			if(h_router.function) return h_router.function(z_stream as WritableFunction & ReadableFunction);
		}
		// iterable
				...
				if(stream) {
					...
					k_self._f_start = () => {
						...
						ds_import.pipe(new stream.Writable({
							_write(g_item: ObjectType, s_encoding: string, fke_write: ErrCallback) {
								// perform transform
