...
class dictionary_thirteen_chapter extends interfaces.dictionary {
	...
	produce_node(i_term) {
		...
		if(i_term < 2) {
			...
			if(i_term <= h_range_oa.hi) {
				...
				throw new RangeError('term id exceeds node id range');
			}
		}
	}

	produce_literal(i_term) {
		let {
			ranges: {
				literals_plain: h_range_lp,
				literals_languaged: h_range_ll,
	...
	find_hop(s_ct) {
		...
		let h_word = this.ct_node_to_word(s_ct);
	...
	find_object_literal(s_ct) {
		let h_chapters = this.chapters;

		// create word from tts node
		let h_word = this.ct_literal_to_word(s_ct);
	...
	ct_node_to_word(s_ct) {
		// iriref
		if('>' === s_ct[0]) {
			// construct iri
			let p_iri = s_ct.slice(1);
		...
		// prefixed name
			...
			let [s_user_prefix, s_suffix] = s_ct.split(':');
	...
	ct_literal_to_word(s_ct) {
		// start of content
		let i_content = s_ct.indexOf('"');
		...
		// non-plain literal
			...
			// datatyped literal
				...
				let h_datatype = this.ct_node_to_word(s_ct.slice(1, i_content));
