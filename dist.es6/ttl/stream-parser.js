...
const pairs = function() {
	...
	if(m_iriref_object) {
			...
			// commit object iri as is
					w_object = s_base + m_iriref_object[1];
		}
		// not iriref
		else {
			// prefixed name
	...
	if(m_prefixed_named_object) {
				...
				// commit object iri from resolve prefixed name
						w_object = h_prefix[m_prefixed_named_object[1]] + m_prefixed_named_object[2];
			}
	...
	if(m_numeric_literal) {
					...
					// commit literal
							w_object = {$: parseFloat(m_numeric_literal[1])};
				}
	...
	if(m_boolean_literal) {
						...
						// make literal
								w_object = {$: m_boolean_literal[1]? true: false};
					}
