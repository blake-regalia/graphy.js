	...
	null_terminated_fc_dict: (ab_dict, a_chunk, at_len, ...a_args) => {
		...
		for (let i_word = 0; i_word < n_words;) {
			...
			while (i_word < i_max) {
				...
				let h = decode_pint(ab_dict, i_read);
				let n_shared = h.v;
				i = h.i;
...
const decode_sorted_list = (a, i) => {
	...
	let h_items = decode_pint(a, i);
	let n_items = h_items.v;
	i = h_items.i;
	// top of each byte in item value
	let h_top = decode_pint(a, i);
	let x_top = h_top.v;
	i = h_top.i;
...
const decode = (a, i) => {
	...
	switch (i_type) {
		...
		case I_NULL:
			...
			// positive int
		case I_PINT:
			return decode_pint(a, i);
			// negative int
		case I_NINT:
			{
				let h = decode_pint(a, i);
				let x_value = h.v;i = h.i;
				return {
					v: -x_value,
					i
				};
		...
		case I_UTF_8_STRING:
			{
				let h = decode_pint(a, i);
				let n_bytes = h.v;i = h.i;
				return {
					v: a.slice(i, i + n_bytes).toString('utf-8'),
					i: i + n_bytes
				};
			}
			// utf-16 string
		case I_UTF_16_STRING:
			{
				...
				let n_bytes = h.v;i = h.i;
				return {
					v: a.slice(i, i + n_bytes).toString('utf-16le'),
					i: i + n_bytes
				};
			}
			// hash
		case I_HASH:
			{
				let h_hash = {};
				let h = decode_pint(a, i);
				let n_pairs = h.v;i = h.i;
				for (let i_pair = 0; i_pair < n_pairs; i_pair++) {
					let h1 = decode_pint(a, i);
					let n_key_length = h1.v;
					i = h1.i;
		...
		case I_BUFFER:
			{
				let h = decode_pint(a, i);
				let c_bytes = h.v;i = h.i;
				return {
					v: a.slice(i, i + c_bytes),
					i: i + c_bytes
				};
			}
			// Uint8Array
		case I_UINT_8_ARRAY:
			{
				...
				let c_bytes = h.v;i = h.i;
				return {
					v: new Uint8Array(a.slice(i, i + c_bytes)),
					i: i + c_bytes
				};
			}
			// otherwise
