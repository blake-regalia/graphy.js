		...
		* results() {
			...
			if (HP_USE_SPO === hp_data_use) {
				...
				let {
					gen: f_o,
					data: h_data_c,
				} = this.iterate_c(h_o, 'o', 's_po');
				let s_save_a = h_data_a && h_data_a.save;
				let s_save_b = h_data_b && h_data_b.save;
				let s_save_c = h_data_c && h_data_c.save;
				let k_data_instance_a, k_data_instance_b, k_data_instance_c;
				if (s_save_a) k_data_instance_a = h_s.data.plugin.instance;
				if (s_save_b) k_data_instance_b = h_p.data.plugin.instance;
				if (s_save_c) k_data_instance_c = h_o.data.plugin.instance;
				let b_extend_a = s_mark_s || s_save_a;
				let b_extend_b = s_mark_p || s_save_b;
				let b_extend_c = s_mark_o || s_save_c;
				...
				if (!h_s.range && !h_p.range && h_o.range && k_pattern.length >= 2 && !k_pattern.peek(0).range && !k_pattern.peek(1).range) {
					...
					for (let i_s of f_s()) {
						...
						// store marked
						h_row_a[s_mark_s] = k_graph.s(i_s);
						...
						for (let [i_p, c_offset_data_sp] of f_p(i_s)) {
							...
							// iterate c
							for (let i_o of f_o(i_s, c_offset_data_sp)) {
								let h_row_c = h_row_b;
					...
					if (s_mark_s) {
						if (s_save_a) {
							...
							for (let i_s of f_s()) {
								...
								h_row_a[s_mark_s] = k_graph.s(i_s);
								// store saved
								h_row_a[s_save_a] = k_data_instance_a.load(i_s);
								if (s_mark_p) {
									if (s_save_b) {
										// iterate b
										for (let [i_p, c_offset_data_sp] of f_p(i_s)) {
											...
											if (s_mark_o) {
												if (s_save_c) {
													...
													for (let i_o of f_o(i_s, c_offset_data_sp)) {
														...
														// store saved
														h_row_c[s_save_c] = k_data_instance_c.load(i_o);
														// ref head(s)
			...
			// POS
				...
				let {
					gen: f_s,
					data: h_data_c,
				} = this.iterate_c(h_s, 's', 'p_os');
				let s_save_a = h_data_a && h_data_a.save;
				let s_save_b = h_data_b && h_data_b.save;
				let s_save_c = h_data_c && h_data_c.save;
				let k_data_instance_a, k_data_instance_b, k_data_instance_c;
				if (s_save_a) k_data_instance_a = h_p.data.plugin.instance;
				if (s_save_b) k_data_instance_b = h_o.data.plugin.instance;
				if (s_save_c) k_data_instance_c = h_s.data.plugin.instance;
				let b_extend_a = s_mark_p || s_save_a;
				let b_extend_b = s_mark_o || s_save_b;
				let b_extend_c = s_mark_s || s_save_c;
				// bidirectional set intersection
				if (!h_p.range && !h_o.range && h_s.range && k_pattern.length >= 2 && !k_pattern.peek(0).range && !k_pattern.peek(1).range) {
					...
					for (let i_p of f_p()) {
						let h_row_a = Object.create(h_row__);
						// store marked
						h_row_a[s_mark_p] = k_graph.p(i_p);
						...
						for (let [i_o, c_offset_data_po] of f_o(i_p)) {
							...
							// iterate c
							for (let i_s of f_s(i_p, c_offset_data_po)) {
								let h_row_c = h_row_b;
								// accumulate ids to m1
								a_m1.push(i_s);
							}
					...
					if (s_mark_p) {
						if (s_save_a) {
							...
							for (let i_p of f_p()) {
								...
								h_row_a[s_mark_p] = k_graph.p(i_p);
								// store saved
								h_row_a[s_save_a] = k_data_instance_a.load(i_p);
								if (s_mark_o) {
									if (s_save_b) {
										// iterate b
										for (let [i_o, c_offset_data_po] of f_o(i_p)) {
											...
											if (s_mark_s) {
												if (s_save_c) {
													...
													for (let i_s of f_s(i_p, c_offset_data_po)) {
														...
														// store saved
														h_row_c[s_save_c] = k_data_instance_c.load(i_s);
														// ref head(s)
			...
			// OSP
				...
				let {
					gen: f_p,
					data: h_data_c,
				} = this.iterate_c(h_p, 'p', 'o_sp');
				let s_save_a = h_data_a && h_data_a.save;
				let s_save_b = h_data_b && h_data_b.save;
				let s_save_c = h_data_c && h_data_c.save;
				let k_data_instance_a, k_data_instance_b, k_data_instance_c;
				if (s_save_a) k_data_instance_a = h_o.data.plugin.instance;
				if (s_save_b) k_data_instance_b = h_s.data.plugin.instance;
				if (s_save_c) k_data_instance_c = h_p.data.plugin.instance;
				let b_extend_a = s_mark_o || s_save_a;
				let b_extend_b = s_mark_s || s_save_b;
				let b_extend_c = s_mark_p || s_save_c;
				// bidirectional set intersection
				if (!h_o.range && !h_s.range && h_p.range && k_pattern.length >= 2 && !k_pattern.peek(0).range && !k_pattern.peek(1).range) {
					...
					for (let i_o of f_o()) {
						let h_row_a = Object.create(h_row__);
						// store marked
						h_row_a[s_mark_o] = k_graph.o(i_o);
						...
						for (let [i_s, c_offset_data_os] of f_s(i_o)) {
							...
							// iterate c
							for (let i_p of f_p(i_o, c_offset_data_os)) {
								let h_row_c = h_row_b;
								// accumulate ids to m1
								a_m1.push(i_p);
							}
					...
					if (s_mark_o) {
						if (s_save_a) {
							...
							for (let i_o of f_o()) {
								...
								h_row_a[s_mark_o] = k_graph.o(i_o);
								// store saved
								h_row_a[s_save_a] = k_data_instance_a.load(i_o);
								if (s_mark_s) {
									if (s_save_b) {
										// iterate b
										for (let [i_s, c_offset_data_os] of f_s(i_o)) {
											...
											if (s_mark_p) {
												if (s_save_c) {
													...
													for (let i_p of f_p(i_o, c_offset_data_os)) {
														...
														let h_row_c = Object.create(h_row_b);
														// store marked
														h_row_c[s_mark_p] = k_graph.p(i_p);
														// store saved
														h_row_c[s_save_c] = k_data_instance_c.load(i_p);
														// ref head(s)
		...
		// V*
			...
			if (!f_filter) {
				...
				// yes data
					...
					// evaluate data find
					let h_found = h_data.plugin.find(h_data.action, i_start, i_stop);
					// mk entity generator
					let f_gen = function*() {
						// id list
						if (h_found.ids) {
			...
			// yes filter
				...
				// yes data
					...
					// evaluate data find
					let h_found = h_data.plugin.find(f_action, i_start, i_stop);
					// mk entity generator
					let f_gen = function*() {
						// id list
						if (h_found.ids) {
						...
						// range
							...
							for (let i_e = h_found.start; i_e < h_found.stop; i_e++) {
								// skip entity if filter rejectes reconstructed term
								if (!f_filter(k_graph[s_term](i_e))) continue;
								// otherwise, yield
								yield i_e;
		...
		// no probes
			...
			// no pattern
				...
				if (s_mark) {
					// and yes save
					if (s_save) {
						let k_instance = h_head.data.plugin.instance;
						for (let i_e of f_e()) {
							let h_row = new Row();
							h_row[s_mark] = k_graph[s_term](i_e);
							h_row[s_save] = k_instance.load(i_e);
