														...
														block() {
																...
																block: while (i < n) {
																		...
																		if (m_graph_iriref_e_graph) {
																			...
																			if (!this.base_url || R_IRI_ABSOLUTE.test(s_iri)) {
																				// set graph
																				this.graph = new NamedNode(s_iri);
																			}
																			// relative iri
																			else {
																				// make graph
																				switch (s_iri[0]) {
																					case '#':
																						this.graph = new NamedNode(this.base_url + s_iri);
																						break;
																					case '?':
																						this.graph = new NamedNode(this.base_url.replace(/(\?.*)?$/, s_iri));
																						break;
																					case '/':
																						// relative to scheme
																						if ('/' === s_iri[1]) {
																							this.graph = new NamedNode(this.base_url_scheme + F_DOT_SEGMENTS(s_iri.substr(1)));
																						}
																						// relative to root
																						else {
																							this.graph = new NamedNode(this.base_url_root + F_DOT_SEGMENTS(s_iri));
																						}
																						break;
																						// empty
																					case undefined:
																						// identity
																						this.graph = new NamedNode(this.base_url);
																						break;
																						// dot segment
																					case '.':
																						// prepend so it is relative to root
																						...
																						// relative to path
																					default:
																						this.graph = new NamedNode(this.base_url_root + F_DOT_SEGMENTS(this.base_url_path + s_iri));
																				}
																			}
																															...
																															if (m_iriref_subject) {
																																...
																																if (m_prefixed_named_subject) {
																																	...
																																	if (m_graph_iriref_graph) {
																																		...
																																		// ref iri
																																		let s_iri = m_graph_iriref_graph[1];
																																																																	...
																																																																	post_object() {
																																																																			...
																																																																			post_object: while (i < n) {
																																																																					...
																																																																					if (`,` === x) {
																																																																						...
																																																																						return (H_DEFAULT_GRAPH === this.graph) ? this.block() : this.statement();
