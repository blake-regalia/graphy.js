...
Object.assign(Parser.prototype, {
	...
	next_prefix_id() {
		...
		if (0x10000 === this.prefix_id) {
			// reassign id generator
			this.next_prefix_id = function() {
				// force lo bits to skip 0, 1, and 2
				let x_lo = this.prefix_id % 0xffff;
				while (x_lo < 2) x_lo = (this.prefix_id++) % 0xffff;

				// start hi bits off at 4 to avoid collision w/ 0, 1, 2, and 3
				return String.fromCharCode((this.prefix_id >> 0x10) + 3) + String.fromCharCode(x_lo);
			};

			// return next id from new generator
			return this.next_prefix_id();
		}

		// single character generator initially
		return String.fromCharCode(this.prefix_id++);
	},
	// 1 to 2 character id generator (limited to ~4.294 billion unique ids)
	next_label_id() {
		// exceeded single character range
		if (0x10000 === this.label_id) {
			// reassign id generator
			this.next_label_id = function() {
				// force lo bits to skip 0, 1, and 2
				let x_lo = this.label_id % 0xffff;
				while (x_lo < 2) x_lo = (this.label_id++) % 0xffff;

				// start hi bits off at 4 to avoid collision w/ 0, 1, 2, and 3
				return String.fromCharCode((this.label_id >> 0x10) + 3) + String.fromCharCode(x_lo);
			};
		...
		// single character generator initially
		return String.fromCharCode(this.label_id++);
	},
		...
		if (i < n) {
			...
			if (0 === i) {
				...
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'crc8');
				}
			}
		}

		// save state before pausing
		this.state = this.crc8;
	...
	crc16(a, n, i) {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'crc16');
				}
			}
		}

		// save state before pausing
		this.state = this.crc16;
	...
	crc32(a, n, i) {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'crc32');
				}
			}
		}

		// save state before pausing
		this.state = this.crc32;
	...
	control(a, n, i) {
		...
		if (n - i >= 4) {
			...
			if (a[i] === 0x24 && a[i + 1] === 0x48 && a[i + 2] === 0x44 && a[i + 3] === 0x54) {
				...
				// next state
				return this.control_type(a, n, i);
			}
		} // 0
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'control');
				}
			}
		}
	...
	control_type(a, n, i) {
		...
		if (i < n) {
			let x_type = a[i++];
			let x_crc = this.crc;
			this.crc = (A_CRC_16[(x_crc ^ x_type) & 0xff] ^ x_crc >>> 8) & 0xffff;
			...
			return this.control_format(a, n, i);
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'control_type');
				}
			}
		}

		// save state before pausing
		this.state = this.control_type;
	...
	control_format(a, n, i) {
		...
		if (-1 !== i_null) {
			let x_crc = this.crc;
			for (let i_b = i; i_b <= i_null; i_b++) {
				x_crc = (A_CRC_16[(x_crc ^ a[i_b]) & 0xff] ^ x_crc >>> 8) & 0xffff;
			}
			this.crc = x_crc;
			// decode string from buffer
			let s_format = decode_utf_8(a.slice(i, i_null));
			...
			// update index
			i = i_null + 1;
			return this.control_properties(a, n, i);
		} // 0
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'control_format');
				}
			}
		}

		// save state before pausing
		this.state = this.control_format;
	...
	control_properties(a, n, i) {
		...
		if (-1 !== i_null) {
			...
			this.crc = x_crc;
			// decode string from buffer
			let s_properties = decode_utf_8(a.slice(i, i_null));
			let h_properties = {};
			...
			// next state
			this.nested.push(this.type.state);
			return this.crc16(a, n, i);
		} // 0
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'control_properties');
				}
			}
		}

		// save state before pausing
		this.state = this.control_properties;
	...
	header(a, n, i) {
		...
		if (n - i >= n_header) {
			...
			// user wants header
			if (this._header) {
				let s_header = decode_utf_8(a.slice(i, i + n_header));
				this._header(s_header);
			}

			// advance index
			i += n_header;
			return this.control(a, n, i);
		}
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'header');
				}
			}
		}
	...
	dict_sect(a, n, i) {
		...
		if (i < n) {
			let x = a[i++];
			let x_crc = this.crc;
			this.crc = A_CRC_8[(x_crc ^ x) & 0xff] & 0xff;
			...
			// dict section plain front coding
				...
				return this.dict_sect_front_coding_string_count(a, n, i);
			} else {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'dict_sect');
				}
			}
		}

		// save state before pausing
		this.state = this.dict_sect;
	...
	dict_sect_front_coding_string_count(a, n, i) {
		this.vbyte_key = 'string_count';
		this.nested.push('dict_sect_front_coding_buffer_length');
		return this.vbyte_8(a, n, i);
	},

	dict_sect_front_coding_buffer_length(a, n, i) {
		this.vbyte_key = 'buffer_length';
		this.nested.push('dict_sect_front_coding_block_size');
		return this.vbyte_8(a, n, i);
	},

	dict_sect_front_coding_block_size(a, n, i) {
		this.vbyte_key = 'block_size';
		this.nested.push('dict_sect_front_coding_crc8');
		return this.vbyte_8(a, n, i);
	},

	dict_sect_front_coding_crc8(a, n, i) {
		this.nested.push('dict_sect_front_coding_log_sequence_open');
		return this.crc8(a, n, i);
	},


	dict_sect_front_coding_log_sequence_open(a, n, i) {
		...
		if (n - i >= 2) {
			...
			if (a[i++] === 1) {
				// update crc
				let x_crc = this.crc;
				x_crc = A_CRC_8[(x_crc ^ 1) & 0xff] & 0xff;

				// number of bits needed to store each element (depending on range of ids)
				let x_bits_per_item = a[i++];

				// update crc
				this.crc = A_CRC_8[(x_crc ^ x_bits_per_item) & 0xff] & 0xff;

				// save to instance
				this.bits_per_item = x_bits_per_item;

				this.vbyte_key = 'log_sequence_entries';
				this.nested.push('dict_sect_front_coding_log_sequence_number_entries');
				return this.vbyte_8(a, n, i);
			}
			// unexpected type id
			else {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'dict_sect_front_coding_log_sequence_open');
				}
			}
		}

		// save state before pausing
		this.state = this.dict_sect_front_coding_log_sequence_open;
	...
	dict_sect_front_coding_log_sequence_number_entries(a, n, i) {
		this.nested.push('dict_sect_front_coding_log_sequence_data');
		return this.crc8(a, n, i);
	},


	dict_sect_front_coding_log_sequence_data(a, n, i) {
		...
		if (n - i >= n_bytes) {
			...
			// start new crc 32
			let x_crc = -1;
			for (let i_b = i; i_b < i + n_bytes; i_b++) {
				x_crc = A_CRC_32[(x_crc ^ a[i_b]) & 0xff] ^ x_crc >>> 8;
			}
			this.crc = x_crc;

			// advance pointer
			i += n_bytes;
			this.nested.push('dict_sect_front_coding_string_prep');
			return this.crc32(a, n, i);
		}
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'dict_sect_front_coding_log_sequence_data');
				}
			}
		}

		// save state before pausing
		this.state = this.dict_sect_front_coding_log_sequence_data;
	...
	dict_sect_front_coding_string_prep(a, n, i) {
		// crc 32 starts at -1
		this.crc = -1;
		return this.dict_sect_front_coding_string(a, n, i);
	},


	dict_sect_front_coding_string(a, n, i) {
		...
		while (i < n) {
			...
			this.crc = x_crc;
			// decode string from buffer
			let s_chunk = decode_utf_8(a.slice(i, i_null));
			...
			if (s_term[0] === '"') {
				...
				let a_oob_matches = s_content.match(/[^\u0000-\u007f]/g);
				if (a_oob_matches) {
					...
					let n_utf8_len = s_content.length + (n_oobs * 1.9395);

					// estimate utf16 length (add weighted average probability of exceeding 2 bytes)
					let n_utf16_len = (s_content.length * 2) + (n_oobs * 1.8858);

					// encode in whichever probably saves more space
					ab_content = n_utf8_len <= n_utf16_len ? encode_utf_8(s_content) : encode_utf_16(s_content);
				}
				// all characters can be encoded in utf8
				else {
					ab_content = encode_utf_8(s_content);
				};
				...
				// literal is datatyped
					...
					// unable to create prefix for null iri
					else {
			...
			if (this.string_seen < this.string_count) {
				...
				// more remain in block
					...
					while (c_b < n_r) {
						...
						// update crc
						x_crc = A_CRC_32[(x_crc ^ x) & 0xff] ^ x_crc >>> 8
					...
					if (b_consumed) {
						...
						this.reuse_length = x_value;

						continue;
					}
					// more bytes needed
					else {
						// save vbyte value to property reuse_length
						this.vbyte_key = 'reuse_length';

						// after vbyte, resume state dict_sect_front_coding_string
						this.nested.push('dict_sect_front_coding_string');

						// save state before pausing
						this.state = this.vbyte_32;
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'dict_sect_front_coding_string');
				}
			}
		}

		// save state before pausing
		this.state = this.dict_sect_front_coding_string;
	...
	dict_sect_front_coding_reuse_length(a, n, i) {
		this.vbyte_key = 'reuse_length';
		this.nested.push('dict_sect_front_coding_string');
		return this.vbyte_32(a, n, i);
	},


	triples(a, n, i) {
		switch (this.format) {
			case P_TRIPLES_BITMAP:
				return this.triples_bitmap(a, n, i);
			case P_TRIPLES_LIST:
				return this.triples_list(a, n, i);
		}
	...
	triples_bitmap(a, n, i) {
		...
		if (i < n) {
			...
			this.crc = A_CRC_8[(x_crc ^ x) & 0xff] & 0xff;
			if (x === 1) {
				this.vbyte_key = 'bit_length';
				this.nested.push('triples_bitmap_preamble');
				return this.vbyte_8(a, n, i);
			} else {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'triples_bitmap');
				}
			}
		}

		// save state before pausing
		this.state = this.triples_bitmap;
	...
	triples_bitmap_preamble(a, n, i) {
		this.nested.push('triples_bitmap_bits_prep');
		return this.crc8(a, n, i);
	},
	...
	triples_bitmap_bits_prep(a, n, i) {
		...
		return this.triples_bitmap_bits(a, n, i);
	},

	triples_bitmap_bits(a, n, i) {
		...
		if (n - i >= n_bytes_remain) {
			// consume entire payload
			a_bytes = a.slice(i, i + n_bytes_remain);

			// update crc
			for (let i_b = i; i_b < i + n_bytes_remain; i_b++) {
				x_crc = A_CRC_32[(x_crc ^ a[i_b]) & 0xff] ^ x_crc >>> 8;
			}
		...
		if (!this.bitmap_section_z) {
			...
			let n_bytes = a_bytes.length;
			for (let i_byte = 0; i_byte < n_bytes; i_byte++) {
				let x_byte = a_bytes[i_byte];
				// end of siblings
			...
			if (b_fragmented) {
				...
				if (v8 && this.approaching_heap_limit(this.bytes_consumed)) {
					...
					// move to typed array
					let ab_idx = i_soal <= 0x100 ?
						new Uint8Array(a_idxs.length) :
						(i_soal <= 0x10000 ?
							new Uint16Array(a_idxs.length) :
							new Uint32Array(a_idxs.length));
					ab_idx.set(a_idxs);
					...
					if (this.graph.idx_sp) {
						...
						let ab_idx_merge = this.graph.idx_sp = i_soal <= 0x100 ?
							new Uint8Array(a_idxs.length + ab_idx_sp.length) :
							(i_soal <= 0x10000 ?
								new Uint16Array(a_idxs.length + ab_idx_sp.length) :
								new Uint32Array(a_idxs.length + ab_idx_sp.length));
			...
			for (let i_byte = 0; i_byte < n_bytes; i_byte++) {
				...
				if (x_byte & 1) {
					...
					if (!--n_bs) {
						...
						let ab_idxs_c = i_soal <= 0x100 ?
							new Uint8Array(a_idxs_c.length) :
							(i_soal <= 0x10000 ?
								new Uint16Array(a_idxs_c.length) :
								new Uint32Array(a_idxs_c.length))
		...
		if (!b_fragmented) {
			...
			if (this.bitmap_section_z) {
				...
				this.nested.push('triples_bitmap');
			}
			// finished with bitmaps, push next state
			else {
				this.nested.push('triples_bitmap_array');
			}
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'triples_bitmap_bits');
				}
			}
		}

		// save state before pausing
		this.state = this.triples_bitmap_bits;
	...
	triples_bitmap_array(a, n, i) {

		// need at least 2 bytes
		if (n - i >= 2) {
			let x = a[i++];
			let x_crc = this.crc;
			x_crc = A_CRC_8[(x_crc ^ x) & 0xff] & 0xff;
			...
			if (x === 1) {
				...
				this.crc = A_CRC_8[(x_crc ^ x_bits_per_item) & 0xff] & 0xff;
				this.bits_per_item = x_bits_per_item;
				this.vbyte_key = 'bitmap_entry_size';
				this.nested.push('triples_bitmap_array_preamble');
				return this.vbyte_8(a, n, i);
			}
			// uint32 format
			else if (x === 2) {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'triples_bitmap_array');
				}
			}
		}

		// save state before pausing
		this.state = this.triples_bitmap_array;
	...
	triples_bitmap_array_preamble(a, n, i) {
		this.nested.push('triples_bitmap_array_entries_prep');
		return this.crc8(a, n, i);
	},

	triples_bitmap_array_entries_prep(a, n, i) {
		// crc 32 starts at -1
		this.crc = -1;
		return this.triples_bitmap_array_entries(a, n, i);
	},


	triples_bitmap_array_entries(a, n, i) {
		// ref graph
		let k_graph = this.graph;
		...
		if (!b_section_z) {
			...
			if (!ab_array) {
				...
				ab_array = k_graph.data_sp = x_mask <= 0x100 ?
					new Uint8Array(this.bitmap_entry_size) :
					(x_mask <= 0x10000 ?
						new Uint16Array(this.bitmap_entry_size) :
						new Uint32Array(this.bitmap_entry_size));
			}
		...
		// HDT "array-Z"
			...
			if (!ab_array) {
				// create data array-buffer
				ab_array = k_graph.data_s_po = x_mask <= 0x100 ?
					new Uint8Array(this.bitmap_entry_size) :
					(x_mask <= 0x10000 ?
						new Uint16Array(this.bitmap_entry_size) :
						new Uint32Array(this.bitmap_entry_size));
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'triples_bitmap_array_entries');
				}
			}
		}

		// save state before pausing
		this.state = this.triples_bitmap_array_entries;
	...
	vbyte_8(a, n, i) {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'vbyte_8');
				}
			}
		}

		// save state before pausing
		this.state = this.vbyte_8;
	...
	vbyte_32(a, n, i) {
		...
		if (i < n) {
			...
			if (0 === i) {
				// we've exceeded the maximum buffer size
				if (n > this.max_buffer_size) {
					return this.parse_error(a, n, i, 'vbyte_32');
				}
			}
		}
	...
	transfer_dict_into_graph(s_section_code, a_dict, a_ref, n_dict_length) {
		...
		let ab_ref = n_dict_length <= 0x100 ?
			new Uint8Array(a_dict.length + 1) :
			(n_dict_length <= 0x10000 ?
				new Uint16Array(a_dict.length + 1) :
				new Uint32Array(a_dict.length + 1));
	...
	transfer_fc_dict_into_graph(s_section_code, a_dict, n_dict_length, n_max_word_length) {
		...
		let at_lens = n_max_word_length <= 0x100 ?
			new Uint8Array(a_dict.length) :
			(n_max_word_length <= 0x10000 ?
				new Uint16Array(a_dict.length) :
				new Uint32Array(a_dict.length))
		...
		// a_ref.length = 0;
		at_ref = i_write <= 0x100 ?
			new Uint8Array(a_ref.length) :
			(i_write <= 0x10000 ?
				new Uint16Array(a_ref.length) :
				new Uint32Array(a_ref.length))
		at_ref.set(a_ref);
