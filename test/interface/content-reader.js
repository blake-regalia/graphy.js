...
const bind = (k_reader, a_events_capture, fke_test) => {
...
const summarize_events = (a_events_capture, fke_test) => {
	let h_events = a_events_capture.reduce((h_out, s_event) => ({
		...h_out,
		[s_event]: [],
	}), {});
...
const graphy_reader_interface = ({
	...
	describe('transform (no input)', () => {
		...
		describe('inline events', () => {
			for(let s_title in h_cases) {
				...
				it(s_title, (fke_test) => {
					// create reader
					let k_reader = f_reader(summarize_events(a_events_capture, (e_read, h_events) => {
						if(e_read) return fke_test(e_read);
	...
	describe('input string', () => {
		...
		it('inline events', ((fke_test) => {
			f_reader({
				input: {string:s_input},
				...summarize_events(a_events_capture, (e_read, h_events) => {
					if(e_read) return fke_test(e_read);
