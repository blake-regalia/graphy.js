	...
	graphy(h_graph, (q_graph) => {
		...
		describe('#node', () => {
			...
			q_graph.network('ns:', (k_banana) => {
				...
				it('contains @type property', () => {
					deep(k_banana['@type'], ['vocab://ns/Fruit']);
				});
		...
		describe('graphy collection', () => {
			...
			q_graph.network('ns:', (k_banana) => {
				...
				it('serializes to n3', () => {
					eq(k_banana.stages.$nquad, '[rdf:first ns:FindSpace;rdf:rest (plant:Seed plant:Grow plant:Harvest)]');
				});

				it('serializes to nquad', () => {
					eq(k_banana.stages.$nquad, '_:b81 <http://www.w3.org/1999/02/22-rdf-syntax-ns#first> <vocab://ns/FindSpace>;<http://www.w3.org/1999/02/22-rdf-syntax-ns#rest> (<vocab://plant/Seed> <vocab://plant/Grow> <vocab://plant/Harvest>)');
				});
