...
## Values
Definitions for the expected types on the values supplied within concise pairs hashes.
...
A value that must be one of the following types:
 ...
 - `Array<`[`ConciseObjectItem`](#value_concise-object-item)`> | Set<`[`ConciseObjectItem`](#value_concise-object-item)`>` -- represents a list of objects, each one belonging to the same graph, subject, and predicate.
